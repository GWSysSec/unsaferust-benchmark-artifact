...
fn test_diff_options_include_ignored_and_recurse_dirs() {
    ...
    // Not including ignored: no ignored files appear.
    ...
    // Untracked recursion on: should include at least one file under untracked_dir.
    let has_untracked_file = paths_no_ign
        .iter()
        .any(|p| p.starts_with("untracked_dir/"));
    assert_eq!(has_untracked_file, true);

    // Now include ignored with recursion.
...
fn test_diff_options_pathspec_and_disable_pathspec_match() {
    ...
    // Exact pathspec with disable_pathspec_match(true) still matches literal name.
    ...
    // Without pathspec both files show.
...
fn test_diff_options_include_typechange_and_unmodified_flags_compile() {
    // Exercise include_typechange / include_typechange_trees / include_unmodified.
    // We don't assert a specific semantic outcome for these flags (the shape of the
    // result depends on libgit2 internals), but we do assert the API is usable and
    // returns a valid diff with correct delta counts in a simple scenario.
    ...
    // Flipping typechange-related flags shouldn't error.
...
fn test_diff_options_all_builder_methods_chain() {
    // Exercise every targeted builder method in a single fluent chain.
    let mut opts = DiffOptions::new();
    ...
    // Modify one.txt and create an untracked file.
    ...
    // two.txt should not appear — unchanged and we didn't include_unmodified.
    let has_two = paths.iter().any(|(_, p)| p == "two.txt");
    assert_eq!(has_two, false);
    // Total of exactly 2 change entries in this simple scenario.
