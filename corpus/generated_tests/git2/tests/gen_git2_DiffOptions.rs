...
fn test_diff_options_prefixes_and_context_lines() {
    ...
    // Custom prefixes and context_lines = 0.
    ...
    // With context_lines=0, "line 0" (unchanged) should not be present.
    assert_eq!(patch.contains("line 0\n"), false);
    assert_eq!(patch.contains("line 9\n"), false);

    // Default prefixes with default context: should contain a/ and b/.
...
fn test_diff_options_force_binary_and_force_text() {
    ...
    // force_binary: patch output says "Binary files differ".
    ...
    // Should not contain textual "rust world" line.
    assert_eq!(patch_bin.contains("rust world"), false);

    // force_text: treat as text.
...
fn test_diff_options_id_abbrev_and_max_size() {
    ...
    // id_abbrev = 40 means full oid shown on index lines.
    ...
    // id_abbrev = 7 (the default-ish abbreviated length).
    ...
    // Extract the index line hashes and check lengths.
    ...
    // max_size(1): treat files larger than 1 byte as binary.
    ...
    // max_size(0) or very large: normal text diff.
...
fn test_diff_options_show_untracked_content() {
    ...
    // Untracked file with distinctive content.
    write_file(&dir, "newfile.txt", b"UNTRACKED_CONTENT_MARKER\n");

    // Without show_untracked_content: untracked file appears but no content lines.
    ...
    // With show_untracked_content: content should appear in patch.
...
fn test_diff_options_all_remaining_builder_methods_chain() {
    // Exercise remaining targeted methods in a fluent chain.
    let mut opts = DiffOptions::new();
    ...
    // context_lines = 5 is > 0, so we should see unchanged neighbors.
