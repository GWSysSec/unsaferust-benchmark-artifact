...
fn test_merge_options_fail_on_conflict() {
    ...
    // Normal merge: succeeds but reports conflicts
    ...
    // fail_on_conflict=false: same behavior as default
    ...
    // fail_on_conflict=true: merge returns an error
...
fn test_merge_options_conflict_styles_and_stages() {
    ...
    // stages should differ in content id
...
fn test_merge_options_all_builder_methods_chain() {
    // Exercise every builder method in a single chain.
    ...
    let reference: &mut MergeOptions = opts
        ...
        .patience(true);
    // The chain returns a &mut reference usable further.
    reference.rename_threshold(75);

    // Now exercise these settings in a real merge scenario with
    // non-conflicting changes across two files (one modified, one added).
    ...
    // Stage 0 means merged (no conflicts at these paths).
