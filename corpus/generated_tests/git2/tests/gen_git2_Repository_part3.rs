...
fn test_merge_analysis() {
    ...
    // Up-to-date: merging current HEAD with itself
    ...
    // From HEAD=c2, merging ac1 (older) -> up-to-date
    ...
    // Reset HEAD to c1; merging ac2 -> fast-forward
...
fn test_merge_basic_then_cleanup() {
    ...
    // Build a sibling commit that doesn't move HEAD or workdir.
    let odb = repo.odb().unwrap();
