...
fn test_status_unwrap_on_complete_request() {
    ...
    assert_eq!(bytes_parsed, buf.len() - 5); // body not included in parsed bytes
...
fn test_status_is_partial_on_incomplete_chunk_size() {
    ...
    // Zero chunk (end of chunked encoding)
    let buf3 = b"0\r\n";
...
fn test_status_unwrap_panics_on_partial() {
    ...
    assert!(result.is_partial());
    // This should panic because the status is partial
    let _ = result.unwrap();
}
...
fn test_status_methods_with_parse_headers() {
    ...
    // bytes_parsed should be the position after the final \r\n\r\n
...
fn test_status_complete_and_partial_multi_step_workflow() {
    // Simulate incremental parsing of a request
