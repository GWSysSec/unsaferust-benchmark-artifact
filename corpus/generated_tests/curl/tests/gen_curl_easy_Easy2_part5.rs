...
fn test_connection_lifecycle_workflow() {
    ...
    // fresh_connect / forbid_reuse toggling
...
fn test_ip_resolve_workflow() {
    ...
    // Use Any (most universally supported variant) and exercise clone trait
    let r = IpResolve::Any;
    let r2 = r.clone();
    let r3 = r2.clone();
    ...
    // Combine with related connection options
...
fn test_ssl_client_cert_and_key_workflow() {
    ...
    // Client cert path — accepts &str via AsRef<Path>
...
fn test_ca_blob_workflow() {
    ...
    // Combine with the rest of connection-lifecycle options in one handle
