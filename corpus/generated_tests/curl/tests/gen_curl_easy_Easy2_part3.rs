...
fn test_easy2_auth_options() {
    ...
    // pre-state: no IO has happened yet
    assert_eq!(easy.get_ref().data.len(), 0);
    assert_eq!(easy.get_ref().calls, 0);

    // password setter accepts a variety of inputs
    ...
    // http_auth with default Auth (no methods enabled = CURLAUTH_NONE)
    let auth_default = Auth::new();
    assert!(easy.http_auth(&auth_default).is_ok());

    // a fresh second Auth should also be acceptable
    let auth_second = Auth::new();
    assert!(easy.http_auth(&auth_second).is_ok());

    // unrestricted_auth: toggle multiple times
    assert!(easy.unrestricted_auth(true).is_ok());
    assert!(easy.unrestricted_auth(false).is_ok());
    assert!(easy.unrestricted_auth(true).is_ok());

    // aws_sigv4 may not be compiled into older libcurl builds.
    // Call it but do not assert the result; we only need to exercise the path.
    let _aws_result = easy.aws_sigv4("aws:amz:us-east-1:s3");

    // post-state: still no network activity
    assert_eq!(easy.get_ref().data.len(), 0);
    assert_eq!(easy.get_ref().calls, 0);
}
...
fn test_easy2_proxy_auth_options() {
    ...
    // proxy_username with several values
    ...
    // Re-applying same setters back to back is valid
...
fn test_easy2_redirects_and_referer() {
    ...
    // alternating again to ensure idempotence of setters
    assert!(easy.autoreferer(false).is_ok());
...
fn test_easy2_combined_workflow() {
    ...
    // Realistic configuration mixing many setters
    ...
    // Reconfigure to confirm setters can be re-applied without errors
    assert!(easy.password("changed").is_ok());
    assert!(easy.max_redirections(0).is_ok());
    assert!(easy.autoreferer(false).is_ok());

    // post-state: no perform() called, so handler must be untouched
    assert_eq!(easy.get_ref().data.len(), 0);
    assert_eq!(easy.get_ref().calls, 0);
}