...
fn test_ssl_and_misc_error_classifiers_all_false() {
    ...
    // For a URL-malformat error, none of these SSL / encoding / conversion
...
fn test_classifiers_idempotent_on_bad_scheme() {
    ...
    // Each classifier must return the same result on repeated calls.
...
fn test_classifiers_mutually_exclusive() {
    ...
    // Sanity: each error individually is also not in any of these specific kinds
...
fn test_classifiers_on_localhost_connection_failure() {
    ...
    // A plain TCP connection failure to localhost:1 is not any of these SSL/auth/encoding
    // related kinds.
    assert_eq!(err.is_recv_error(), false);
    assert_eq!(err.is_ssl_certproblem(), false);
    assert_eq!(err.is_ssl_cipher(), false);
