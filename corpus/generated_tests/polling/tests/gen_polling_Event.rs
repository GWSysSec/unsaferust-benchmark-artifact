...
fn event_all_constructor() {
    ...
    // is_err / is_connect_failed should not indicate error on a fresh event
...
fn event_set_and_with_interrupt() {
    ...
    // On platforms supporting it, is_interrupt becomes true; on others it remains false.
    // At minimum, calling these should not panic and toggling off should give false.
...
fn event_combined_flags() {
    ...
    // If priority is supported on this platform, the flag should differ from plain.
    if int_active {
    ...
    // Neither should be considered an error.
    assert_ne!(ev.is_err(), Some(true));
    assert_ne!(ev.is_connect_failed(), Some(true));
}
...
fn event_error_predicates_on_normal_events() {
    ...
    for ev in [&r, &w, &a, &n] {
        // is_err / is_connect_failed return Option<bool>; on a fresh event neither should be Some(true).
        assert_ne!(ev.is_err(), Some(true));
        assert_ne!(ev.is_connect_failed(), Some(true));
        assert!(!ev.is_interrupt());
        assert!(!ev.is_priority());
