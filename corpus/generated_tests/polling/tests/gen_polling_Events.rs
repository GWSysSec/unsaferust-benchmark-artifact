...
fn capacity_is_positive_and_stable() {
    ...
    assert!(cap.get() >= 1);
    assert_ne!(cap.get(), 0);
    // NonZeroUsize guarantees non-zero
    assert_eq!(cap, events.capacity());
    assert_eq!(events.len(), 0);
    assert!(events.is_empty());
    // Capacity should not change from simply reading len/is_empty
...
fn capacity_unchanged_after_clear() {
    ...
    assert!(events.is_empty());
    assert_eq!(events.iter().count(), 0);
    // Repeated clears shouldn't shrink capacity
    events.clear();
    events.clear();
...
fn capacity_accommodates_events_after_wait() -> io::Result<()> {
    ...
    // Capacity must be at least the number of events it holds.
