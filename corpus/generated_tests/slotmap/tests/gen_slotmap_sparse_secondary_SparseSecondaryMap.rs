...
fn test_sparse_secondary_map_entry_api() {
    ...
    // or_insert on occupied (should not overwrite)
...
fn test_sparse_secondary_map_removed_primary_key() {
    ...
    // Secondary still has the entry until explicitly removed or the key is reused
    // The value may still be accessible with the old key
    // This tests that the sparse secondary map handles stale keys gracefully
