...
fn test_with_key_is_empty_capacity_reserve() {
    ...
    // Re-reserve a smaller amount should not shrink
...
fn test_insert_with_key_self_reference() {
    ...
    // All self-references still valid after multiple inserts
    assert_eq!(sm[k1].0, k1);
    assert_eq!(sm[k2].0, k2);
    assert_eq!(sm[k3].0, k3);
    assert_eq!(sm.len(), 3);
...
fn test_try_insert_with_key_ok_and_err() {
    ...
    // Failed try must not grow the map
    assert_eq!(sm.len(), 1);
    assert_eq!(sm[k1], 7);

    // Closure receives a valid key that would become real on Ok
    let r2: Result<DefaultKey, &'static str> = sm.try_insert_with_key(|k| {
        // After success, this key should address value 99
        let _ = k;
        Ok(99)
    });
    assert!(r2.is_ok());
    ...
    // Another error does not affect surviving data
...
fn test_retain_even_values_then_clear() {
    ...
    // Retain even values only
    sm.retain(|_k, v| *v % 2 == 0);
    assert_eq!(sm.len(), 10);
    assert_eq!(sm.is_empty(), false);

    // Even indices kept, odd removed
    for (i, k) in keys.iter().enumerate() {
        if i % 2 == 0 {
    ...
    // Retain-all keeps size stable
    sm.retain(|_k, _v| true);
    assert_eq!(sm.len(), 10);

    // Retain-none empties the map
    sm.retain(|_k, _v| false);
    assert_eq!(sm.len(), 0);
    assert!(sm.is_empty());

    // Now verify clear() independently
    for i in 0..5 {
        sm.insert(i * 10);
    }
    assert_eq!(sm.len(), 5);
    ...
    // After clear, we can still insert and keys are fresh-usable
...
fn test_get_disjoint_mut_ok_and_overlap_and_invalid() {
    ...
    *arr[2] = 30;
    ...
    // Overlapping keys must yield None
    ...
    // Invalid key (removed) must yield None
    sm.remove(k4);
    assert_eq!(sm.contains_key(k4), false);
    let invalid = sm.get_disjoint_mut([k1, k4]);
    assert!(invalid.is_none());

    // Original data preserved after failed lookups
    assert_eq!(sm[k1], 10);
    assert_eq!(sm[k2], 20);
    assert_eq!(sm[k3], 30);
    assert_eq!(sm.len(), 3);

    // Single-key disjoint call works
...
fn test_workflow_reserve_insert_with_key_retain_iter_mut() {
    ...
    // All flags now false, so retain(true-only) clears them
