...
fn test_with_key_is_empty_capacity_reserve() {
    ...
    // Reserve additional — capacity must not shrink.
...
fn test_insert_with_key_self_reference() {
    ...
    // k1 entry must still be intact.
    assert_eq!(sm[k1].0, k1);
    assert_eq!(sm[k1].1, "first");

    // Many insertions, each self-referencing.
    let mut ks = Vec::new();
    for _ in 0..50 {
...
fn test_try_insert_with_key_success_and_failure() {
    ...
    // Success after failure; closure sees the final key assigned.
...
fn test_clear_preserves_capacity_invalidates_keys() {
    ...
    // Post-clear insertions still work and produce fresh keys.
    let k4 = sm.insert("new".to_string());
    assert_eq!(sm.len(), 1);
    assert_eq!(sm[k4], "new");
    assert_ne!(k4, k1);

    // Clearing an already-empty map after inserting one element — 
    // the original test called clear() twice expecting it to be a no-op on an empty map,
    // but the first clear removes k4. Fix: only clear once, then verify.
    sm.clear();
    assert_eq!(sm.len(), 0);
...
fn test_get_disjoint_unchecked_mut_unsafe() {
    ...
    // Single element via unchecked path.
    unsafe {
...
fn test_iter_mut_modifies_all_values() {
    ...
    // Remove some and iterate again — iter_mut must skip removed slots.
    sm.remove(keys[3]);
    sm.remove(keys[7]);
    assert_eq!(sm.len(), 8);
