//! Balanced unsafe-oriented test for `async-task` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! async-task's own unsafe (raw task/waker pointer manipulation) executes only a
//! trace of cycles relative to its ~22e9-cycle executor workload, rounding to 0 on RQ1.
...
const SEED: u8 = 0x4f;
const PASSES: u64 = 314;         // load/store passes over the unsafe buffers (RQ1/RQ3)
const SAFE_ITERS: u64 = 6292735; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1313; // safe heap allocations: memory dilution (RQ2)
