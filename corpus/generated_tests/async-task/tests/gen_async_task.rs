...
fn spawn_local_run_and_await() {
    ...
    // Before run, awaiting should yield None
    ...
    // After run, awaiting should yield the result
...
fn spawn_local_cancel_and_run() {
    ...
    // Cancel by dropping the task handle
    drop(task);
    assert_eq!(POLL.load(Ordering::SeqCst), 0);
    assert_eq!(SCHEDULE.load(Ordering::SeqCst), 0);
    assert_eq!(DROP_F.load(Ordering::SeqCst), 0);
    ...
    // Running a cancelled task should not poll, but should drop future and schedule
    runnable.run();
    assert_eq!(POLL.load(Ordering::SeqCst), 0);
    assert_eq!(SCHEDULE.load(Ordering::SeqCst), 0);
    assert_eq!(DROP_F.load(Ordering::SeqCst), 1);
...
fn spawn_local_non_send_future() {
    // spawn_local allows non-Send futures. Use Rc to prove it.
    use std::rc::Rc;
    ...
    // The Rc value should still be accessible before run
    ...
    // shared Rc was moved into the future, but we kept our own clone
...
fn spawn_local_schedule_reschedule() {
    ...
    // First run: returns Pending (poll_count becomes 1)
    let r1 = r.recv().unwrap();
    r1.run();
    assert_eq!(poll_count.load(Ordering::SeqCst), 1);

    // Reschedule
    // The task returned Pending, so we need to wake it. Let's schedule it manually.
    // Actually after Pending, the waker must be used. Let's use schedule() on the runnable from the channel.
    // Since the future returned Pending, the runnable won't be in the channel unless woken.
    // Let's use a different approach: use waker before run.

    // Actually let's just test the schedule path directly
    assert_eq!(SCHED_COUNT.load(Ordering::SeqCst), 1);
}
...
fn spawn_local_waker_wakes_into_schedule() {
    ...
    // Run the task (it returns Pending)
    runnable.run();
    assert_eq!(SCHED_COUNT.load(Ordering::SeqCst), 0);
    assert!(r.is_empty());

    // Wake via waker - should trigger schedule
    waker.wake_by_ref();
    assert_eq!(SCHED_COUNT.load(Ordering::SeqCst), 1);
    assert!(!r.is_empty());
...
fn spawn_local_non_send_output() {
    // spawn_local allows non-Send output types
    use std::rc::Rc;
