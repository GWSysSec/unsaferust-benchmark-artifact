...
fn test_hasher_returns_reference() {
    ...
    // Verify we can use the hasher reference (it exists and is valid)
...
fn test_truncate_shortens_set() {
    ...
    // Truncate to 0 should empty the set
    set.truncate(0);
    assert_eq!(set.len(), 0);
    assert!(set.is_empty());
}
...
fn test_reserve_exact() {
    ...
    // Insert elements and verify they work
    for i in 0..50 {
        set.insert(i);
    }
...
fn test_splice_replace_range() {
    ...
    // Splice out indices 1..3 (values 10, 20) and replace with 100, 200, 300
    ...
    // Set should now be: 0, 100, 200, 300, 30, 40
    assert!(set.contains(&0));
...
fn test_splice_empty_replacement() {
    ...
    // Remove elements at indices 1..4 without replacement
...
fn test_append_merges_sets() {
    ...
    // set1 should have all unique elements
    ...
    // set2 should be empty after append
    assert_eq!(set2.len(), 0);
    assert!(set2.is_empty());
}
...
fn test_take_removes_and_returns_value() {
    ...
    // Taking a non-existent value returns None
    let not_found = set.take("omega");
    assert_eq!(not_found, None);
    assert_eq!(set.len(), 3);

    // Remaining elements are still present
...
fn test_swap_take_removes_value() {
    ...
    // swap_take removes the element and swaps the last element into its place
    let taken = set.swap_take(&20);
    assert_eq!(taken, Some(20));
    assert_eq!(set.len(), 4);
    assert!(!set.contains(&20));

    // The last element (50) should now be at index 1
    ...
    // swap_take on non-existent returns None
...
fn test_shift_take_removes_value_preserving_order() {
    ...
    // shift_take removes the element and shifts subsequent elements to fill the gap
    let taken = set.shift_take(&20);
    assert_eq!(taken, Some(20));
    assert_eq!(set.len(), 4);
    assert!(!set.contains(&20));

    // Order should be preserved: 10, 30, 40, 50
    ...
    // shift_take on non-existent returns None
...
fn test_sorted_by_returns_sorted_iter() {
    ...
    // sorted_by consumes the set and returns an IntoIter in sorted order
...
fn test_combined_workflow_truncate_sort_retain() {
    ...
    // Truncate to 6 elements
    set.truncate(6);
    assert_eq!(set.len(), 6);
    assert!(!set.contains(&88));
    assert!(!set.contains(&99));

    // Retain only values > 5
    set.retain(|&x| x > 5);
    assert!(!set.contains(&1));
    assert!(!set.contains(&3));
    assert!(set.contains(&7));
...
fn test_swap_take_vs_shift_take_ordering() {
    // Demonstrate the difference between swap_take and shift_take
    let mut set_swap: IndexSet<i32> = IndexSet::new();
    let mut set_shift: IndexSet<i32> = IndexSet::new();

    for i in 1..=5 {
        set_swap.insert(i * 10); // 10, 20, 30, 40, 50
        set_shift.insert(i * 10);
    }

    // Remove element at index 1 (value 20) using swap_take
    let sw = set_swap.swap_take(&20);
    assert_eq!(sw, Some(20));
    // Last element (50) fills the gap at index 1
    ...
    // Remove element at index 1 (value 20) using shift_take
...
fn test_splice_with_duplicates_in_replacement() {
    ...
    // Replace range 1..3 with values that include a duplicate of existing element (1)
    // and a duplicate within the replacement (10, 10)
    ...
    // Duplicates should be ignored by the set
    assert!(set.contains(&1));
    assert!(set.contains(&10));
    assert!(set.contains(&20));
    assert!(set.contains(&4));
