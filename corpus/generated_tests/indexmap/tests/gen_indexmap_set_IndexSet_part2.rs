...
fn test_partition_point() {
    ...
    // All elements < 6 are at the beginning
...
fn test_shift_remove_index_basic() {
    ...
    // After shift_remove_index, elements after the removed one shift down
    assert_eq!(set.get_index(0), Some(&"first"));
...
fn test_shift_remove_index_preserves_order() {
    ...
    // Remove index 3 (value 30)
    let removed = set.shift_remove_index(3);
    assert_eq!(removed, Some(30));
    assert_eq!(set.len(), 9);

    // Remove index 5 (was originally value 60, now shifted)
    let removed = set.shift_remove_index(5);
...
fn test_sort_then_binary_search_workflow() {
    ...
    // Before sorting, binary_search may not work correctly
    set.sort_unstable();
...
fn test_sort_remove_and_range_workflow() {
    ...
    // Partition point after removal
    let point = set.partition_point(|&x| x < 10);
    assert_eq!(point, 9); // elements 1..=9 are < 10, that's 9 elements
...
fn test_sorted_unstable_by_into_iter_as_slice() {
    ...
    // as_slice shows remaining elements
