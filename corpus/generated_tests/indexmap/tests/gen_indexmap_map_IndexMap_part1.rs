...
fn test_hasher_returns_valid_build_hasher() {
    ...
    // Verify we can actually use the hasher to hash something
    ...
    // Hasher from a populated map should also work
    let map2 = indexmap! { "a" => 1, "b" => 2 };
    let hasher2 = map2.hasher();
    let mut s = hasher2.build_hasher();
    "a".hash(&mut s);
    let h = s.finish();
    assert_ne!(h, 0_u64.wrapping_add(1).wrapping_sub(1)); // non-trivial check: hash is computed
    // Verify the map still works after borrowing hasher
...
fn test_split_off_divides_map() {
    ...
    // split_off at 0 moves everything
...
fn test_reserve_exact_increases_capacity() {
    ...
    // Insert items up to the reserved amount without reallocation
    for i in 0..100 {
        map.insert(i, i * 2);
    }
    ...
    // Reserve exact with existing elements
...
fn test_try_reserve_exact_success_and_error() {
    ...
    // Try to reserve a reasonable additional amount
    let result2 = map.try_reserve_exact(200);
    assert!(result2.is_ok());
    assert!(map.capacity() >= 250);

    // Try to reserve an absurdly large amount that should fail
    ...
    // Map should still be usable after failed reservation
    assert_eq!(map.len(), 50);
    assert_eq!(map[&0], 1);
}
...
fn test_splice_replaces_range() {
    ...
    // Map should now have: a, x, y, z, d, e
    assert_eq!(map.len(), 6);
...
fn test_append_merges_maps() {
    ...
    // map2 should be drained
    assert_eq!(map2.len(), 0);
    assert!(map2.is_empty());

    // map1 should have all unique keys; duplicate "b" should update value
    assert!(map1.contains_key("a"));
    assert!(map1.contains_key("b"));
    assert!(map1.contains_key("c"));
    assert!(map1.contains_key("d"));
    assert_eq!(map1["a"], 1);
    assert_eq!(map1["b"], 99); // updated from map2
    assert_eq!(map1["c"], 3);
    assert_eq!(map1["d"], 4);
}
...
fn test_remove_entry_swap_removes() {
    ...
    // remove_entry uses swap removal (last element fills the gap)
    let removed = map.remove_entry("b");
    assert_eq!(removed, Some(("b", 20)));
    assert_eq!(map.len(), 3);
    assert!(!map.contains_key("b"));

    // After swap remove of index 1, "d" (last) should now be at index 1
...
fn test_shift_remove_entry_preserves_order() {
    ...
    // Order should be preserved (shift, not swap)
    assert_eq!(map.get_index(0), Some((&"a", &10)));
...
fn test_retain_filters_entries() {
    ...
    // Retain with value-based predicate
...
fn test_sort_by_custom_comparator() {
    ...
    // Sort by value ascending, then by key for ties
...
fn test_sorted_by_consumes_and_returns_sorted_iter() {
    ...
    // Use as_slice to check remaining items
...
fn test_splice_empty_range_inserts() {
    ...
    // Splice with empty range inserts without removing
...
fn test_retain_mutates_values() {
    ...
    map.retain(|k, v| {
        *v += 1; // mutate all values
        *k <= 3  // keep only keys <= 3
    });

    assert_eq!(map.len(), 3);
    // Values were mutated before the filter decision
    assert_eq!(map[&1], 11);
    assert_eq!(map[&2], 21);
    assert_eq!(map[&3], 31);
    assert!(!map.contains_key(&4));
