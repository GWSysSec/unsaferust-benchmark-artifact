...
fn test_slice_get_range_mut_modifications() {
    ...
    // Modify values through the mutable slice
...
fn test_slice_binary_search_by() {
    // Build a map with keys in sorted order for binary search
    let mut map = IndexMap::new();
    for i in 0..10 {
        map.insert(i * 5, format!("v{}", i * 5));
    }
    // Keys: 0, 5, 10, 15, 20, 25, 30, 35, 40, 45
    let slice = map.as_slice();
...
fn test_slice_binary_search_by_key() {
    // Map with string keys but we search by the numeric value
    let mut map = IndexMap::new();
    map.insert("a", 10);
    map.insert("b", 20);
    map.insert("c", 30);
...
fn test_slice_partition_point() {
    ...
    // Values: 0, 6, 12, 18, 24, 30, 36, 42
    let slice = map.as_slice();

    // Partition point where key >= 10
    let pp = slice.partition_point(|k, _| *k < 10);
    assert_eq!(pp, 4); // keys 0,3,6,9 are < 10
    ...
    assert_eq!(pp, 4); // values 0,6,12,18 are < 24
...
fn test_slice_combined_workflow() {
    ...
    // Left should be unmodified
    assert_eq!(map[&0], 0.0);
    assert_eq!(map[&2], 3.0);
    // Modified range
    assert_eq!(map[&3], 9.0);  // 4.5 * 2
    assert_eq!(map[&6], 18.0); // 9.0 * 2
    // After modified range should be unmodified
    assert_eq!(map[&7], 10.5);
    assert_eq!(right.len(), 7);

    // binary_search_by on sorted keys
