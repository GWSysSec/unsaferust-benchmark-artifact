//! Balanced unsafe-oriented test for `pin-project-lite` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! pin-project-lite is a macro crate: its unsafe is emitted into the *consumer*
//! crate, so the library itself compiles ~zero executable unsafe.
...
const SEED: u8 = 0xbe;
const PASSES: u64 = 61;         // load/store passes over the unsafe buffers (RQ1/RQ3)
const SAFE_ITERS: u64 = 13645350; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 70; // safe heap allocations: memory dilution (RQ2)
