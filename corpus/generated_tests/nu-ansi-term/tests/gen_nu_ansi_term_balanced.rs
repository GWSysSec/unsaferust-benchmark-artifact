//! Balanced unsafe-oriented test for `nu-ansi-term` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! nu-ansi-term's only unsafe is in src/windows.rs (#[cfg(windows)]), so a Linux
//! build compiles with zero executable unsafe.
...
const SAFE_ITERS: u64 = 11756400; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1299; // safe heap allocations: memory dilution (RQ2)
