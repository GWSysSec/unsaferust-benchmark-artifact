...
fn gradient_text_basic_properties() {
    ...
    assert_ne!(out.len(), 0);
    // Output should be longer than original because of ANSI escapes
    assert!(out.len() > text.len());
    // Should contain ESC character used in ANSI
    assert!(out.contains('\x1b'));
    // Should contain the original characters in order
    for ch in text.chars() {
        assert!(out.contains(ch), "missing char {:?}", ch);
    }
    // Should contain reset sequence
    assert!(out.contains("\x1b[0m"));
    // Should contain foreground 38;2 truecolor code
    assert!(out.contains("38;2;"));
    // Should contain background 48;2 truecolor code
    assert!(out.contains("48;2;"));
    // Start color red component should appear
...
fn gradient_text_empty_string() {
    ...
    // Empty input should yield no characters or only trivial output
    assert!(!out.contains('H'));
    assert!(!out.contains('a'));
    assert!(out.len() < 32);
    // Non-empty inputs differ significantly from empty
