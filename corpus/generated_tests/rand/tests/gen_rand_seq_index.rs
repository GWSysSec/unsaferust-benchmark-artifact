...
fn sample_weighted_basic_distinct_indices() {
    ...
    // sum of distinct indices in [0,10) is at most 9+8+7+6+5=35
...
fn sample_weighted_zero_amount() {
    ...
    assert_eq!(v.len(), 0);
    assert!(v.is_empty());
    // run twice to check determinism of empty
...
fn sample_weighted_amount_exceeds_length_errors() {
    ...
    // In this version of rand, amount > length does not produce an error;
    // it clamps and returns all available indices.
    ...
    // Ensure subsequent valid call still works
...
fn sample_weighted_skewed_weights_bias() {
    let mut rng = make_rng();
    // Heavy weight on index 0: it should be selected nearly always when amount=1
