...
fn test_entry_or_insert_and_and_modify() {
    ...
    // or_insert on occupied entry returns existing value (does NOT overwrite)
    {
    ...
    // and_modify on vacant: closure NOT called, then or_insert applies
...
fn test_entry_or_insert_with_and_or_default() {
    ...
    // or_insert_with on occupied: closure not called
    ...
    // or_default on a brand-new key uses Default::default()
    ...
    // or_default on existing key returns existing value
...
fn test_entry_or_try_insert_with_success_and_failure() {
    ...
    // Failure path on vacant: nothing inserted
    ...
    // On occupied key: closure not called, returns existing
    let mut called = false;
...
fn test_entry_into_key() {
    ...
    // Vacant: into_key returns the original key
    ...
    // Occupied: into_key returns the supplied key (the one passed to entry)
...
fn test_entry_combined_workflow() {
    // Realistic: counting word frequencies and lazily allocating buckets.
    ...
    // Now use or_try_insert_with: only insert "e" if a parsed condition holds
