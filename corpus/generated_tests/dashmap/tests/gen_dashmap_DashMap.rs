...
fn test_remove_if_and_remove_if_mut() {
    ...
    // Predicate returns false -> not removed
    let r = m.remove_if(&3, |_k, v| *v > 1000);
    assert!(r.is_none());
    assert!(m.contains_key(&3));

    // Predicate returns true -> removed
...
fn test_alter_and_alter_all() {
    ...
    // alter on missing key is a no-op
...
fn test_view_and_try_entry() {
    ...
    // try_entry should succeed when no conflicting locks held
    {
    ...
    // Hold a write reference to a key in the same shard, then try_entry
    // for that key should yield None.
    let key_a = 100;
