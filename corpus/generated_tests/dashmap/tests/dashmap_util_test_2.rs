...
where
    F: FnOnce(C, T) -> T,
{
    // Move the old value out of the mutable reference, compute the replacement,
    // then write the new value back into the same location.
    //
    // This mirrors the behavior needed by these tests for non-Clone values.
    unsafe {
