...
fn test_occupied_entry_into_ref_mutates_in_place() {
    ...
    match map.entry("counter".to_string()) {
        Entry::Occupied(occ) => {
            // get() before consuming via into_ref
...
fn test_occupied_entry_into_key_returns_supplied_key() {
    ...
    match map.entry(supplied) {
        Entry::Occupied(occ) => {
            ...
            // The returned key is the one we passed in (same heap buffer).
            assert_eq!(k.as_ptr(), supplied_ptr);
        }
        Entry::Vacant(_) => panic!("expected Occupied"),
    }

    // The map itself is unchanged: the original key remains stored.
    assert_eq!(map.len(), 2);
...
fn test_occupied_entry_remove_entry_returns_kv_and_removes() {
    ...
    // Removing the last keys via remove_entry one-by-one.
...
fn test_occupied_entry_replace_entry_returns_old_kv() {
    ...
    // Supply a different *equal* key string; replace_entry should return the
    // previously stored key (different buffer than the one we just supplied).
    ...
    match map.entry(new_supplied_key) {
        Entry::Occupied(occ) => {
            ...
            // The returned key is the *previous* (originally stored) key.
...
fn test_occupied_entry_combined_workflow() {
    // Multi-step: build a map, mutate via into_ref, replace one entry, remove another,
    // and inspect into_key on a survivor.
    ...
    // into_key on "w" preserves the supplied key identity.
