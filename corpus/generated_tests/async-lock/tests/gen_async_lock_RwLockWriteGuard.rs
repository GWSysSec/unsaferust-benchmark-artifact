...
fn downgrade_write_to_read() {
    future::block_on(async {
        ...
        // Acquire write, mutate, then downgrade to read.
        ...
        *w = 200;
        ...
        // Should be able to acquire another read concurrently.
        let r2 = lock.read().await;
        assert_eq!(*r2, 200);
        assert_eq!(*r, 200);

        // Write should not be available while reads are held.
        assert!(lock.try_write().is_none());

        drop(r);
        drop(r2);

        // Now writes are possible again.
...
fn downgrade_write_to_upgradable() {
    future::block_on(async {
        ...
        *w = 42;
        ...
        // Other reads should be possible while upgradable held.
        let r = lock.read().await;
        assert_eq!(*r, 42);

        // Another upgradable shouldn't be possible.
        ...
        // Re-upgrade back to write and mutate.
