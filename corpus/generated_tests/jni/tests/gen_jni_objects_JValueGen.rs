...
fn test_type_name_returns_distinct_strings() {
    ...
    // Deterministic (same variant => same name across calls)
...
fn test_to_jni_extracts_primitive_fields() {
    ...
    // jvalue is a union; field access requires unsafe.
    unsafe {
...
fn test_borrow_preserves_primitive_values() {
    ...
    // borrow() takes &self; extracts primitive via borrowed value's accessor.
    {
    ...
    // Originals unchanged because borrow() does not consume.
...
fn test_boundary_values_roundtrip_through_to_jni_and_accessors() {
    // Extreme primitive values survive round-trips through the accessors.
    ...
    // Negative zero, infinity, NaN round-trip through to_jni + field read.
