...
fn test_get_set_field_int() {
    ...
    // Construct AtomicInteger(42) so we know the private `int value` field starts at 42
    let obj = env
        .new_object(
    ...
    // Error: wrong signature (value is int, not long)
...
fn test_set_static_field_object() {
    ...
    // Restore original System.err to avoid affecting subsequent tests
    env.set_static_field(
        "java/lang/System",
        ("java/lang/System", "err", "Ljava/io/PrintStream;"),
    ...
    // Error: field that doesn't exist
...
fn test_rust_field_lifecycle() {
    ...
    // Second borrow works after guard dropped
    {
...
fn test_lock_obj_monitor() {
    ...
    // Basic lock + drop
    {
        ...
        // Independent monitor on obj2 acquires fine
    ...
    // Lock works on other reference types (a JString)
    let s = env.new_string("locked").unwrap();
    {
        let _gs = env.lock_obj(&s).unwrap();
    }

    // Rapid acquire/release cycles — each must succeed
    let mut acquired = 0;
    for _ in 0..16 {
        let _g = env.lock_obj(&obj1).unwrap();
        acquired += 1;
    ...
    // Reentrant locking (Java monitors are reentrant on the same thread)
    let mut depth = 0;
    {
        let _g1 = env.lock_obj(&obj1).unwrap();
        depth += 1;
    ...
    // After all guards dropped, can still lock
...
fn test_ensure_local_capacity_and_refs() {
    ...
    // A range of valid capacities should all succeed
    ...
    // Request capacity for a batch and then actually allocate that many local refs
    ...
    // Verify first, middle, last values
    {
    ...
    // Capacity still adjustable after freeing refs
...
fn test_register_unregister_native_methods() {
    ...
    // Register a dummy native method (will not actually be called, just testing the API)
