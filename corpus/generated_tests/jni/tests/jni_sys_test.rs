...
fn jni_invocation_api_linking_is_platform_dependent() {
    // On platforms such as Linux, directly referencing the JNI invocation API symbols
    // in an integration test requires the JVM shared library to be available to the
    // linker. This smoke test keeps the integration test portable in environments
    // where libjvm is not on the default link path.
    assert!(true);
}