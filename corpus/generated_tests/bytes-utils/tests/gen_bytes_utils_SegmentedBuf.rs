...
fn test_segmented_buf_into_inner_empty() {
    ...
    // Rebuild from the returned VecDeque to confirm it's usable
...
fn test_segmented_buf_into_inner_after_partial_advance() {
    ...
    // Advance past the first segment entirely
    buf.advance(5);
    assert_eq!(buf.remaining(), 11);
    assert_eq!(buf.chunk(), b"second");

    // Advance partially into the second segment
    buf.advance(3);
    assert_eq!(buf.remaining(), 8);
    assert_eq!(buf.chunk(), b"ond");

    let inner: VecDeque<&[u8]> = buf.into_inner();
    // After advancing, consumed segments may be removed
    // The remaining data should be accessible
...
fn test_segmented_buf_into_inner_with_empty_segments() {
    ...
    // remaining should only count non-empty bytes
    assert_eq!(buf.remaining(), 12);
    assert!(buf.has_remaining());
    assert_eq!(buf.chunk(), b"nonempty");

    let inner: VecDeque<&[u8]> = buf.into_inner();
    // All segments should be present in the deque
    let total_bytes: usize = inner.iter().map(|s| s.len()).sum();
    assert_eq!(total_bytes, 12);

    // Verify non-empty segments contain expected data
...
fn test_segmented_buf_into_inner_roundtrip() {
    ...
    // Rebuild a new SegmentedBuf from the inner contents
...
fn test_segmented_buf_into_inner_large_number_of_segments() {
    ...
    // Advance a bit
    buf.advance(1); // consume first segment (1 byte of value 0)
    ...
    // Verify the deque is iterable and well-formed
    assert!(inner.len() >= 1);
...
fn test_segmented_buf_into_inner_fully_consumed() {
    ...
    let inner: VecDeque<&[u8]> = buf.into_inner();
    // After full consumption, inner should have no remaining data
    let total: usize = inner.iter().map(|s| s.len()).sum();
    assert_eq!(total, 0);
    // The deque itself should be empty since segments are consumed
