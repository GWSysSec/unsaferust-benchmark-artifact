...
fn test_headers_mut_insert_and_modify() {
    ...
    // Pre-state: verify initial header exists
    assert_eq!(
    ...
    // Remove a header via headers_mut
    let headers = request.headers_mut();
...
fn test_headers_mut_append_multiple_values() {
    ...
    // HeaderMap::len counts all values
    assert_eq!(request.headers().len(), 3);
...
fn test_extensions_empty_and_read() {
    ...
    // Verify other request properties are intact
...
fn test_into_body_consumes_request() {
    ...
    // Verify pre-state before consuming
...
fn test_map_transforms_body_type() {
    ...
    // Map string body to parsed integer
    let mapped: Request<u64> = request.map(|body| body.parse::<u64>().unwrap());

    // Post-state: body is transformed
    assert_eq!(*mapped.body(), 42u64);
    // Method and URI are preserved
...
fn test_map_string_to_bytes() {
    ...
    // Extensions are preserved through map
...
fn test_map_with_closure_capturing_state() {
    ...
    assert_eq!(mapped.body().len(), 10); // 7 prefix + 3 original
...
fn test_headers_mut_entry_api() {
    ...
    // or_insert on occupied entry should not overwrite
