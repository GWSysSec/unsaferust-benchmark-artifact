//! Balanced unsafe-oriented test for `os_info` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! Every unsafe block in os_info is #[cfg(windows)] (the WinAPI backend), so a
//! Linux build compiles with zero executable unsafe.
...
const SAFE_ITERS: u64 = 11691063; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1309; // safe heap allocations: memory dilution (RQ2)
