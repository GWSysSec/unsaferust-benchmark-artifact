...
fn test_set_global_recorder_success() {
    // set_global_recorder can only succeed once per process, so we test the second call fails.
    // The first call may have already been done by another test in this binary.
    let recorder = TestRecorder::new();
    let result = set_global_recorder(recorder);
    // Either it succeeds (first time) or it fails (already set).
    // We just verify the Result type is correct and usable.
    let was_ok = result.is_ok();
    let was_err = result.is_err();
    assert_ne!(was_ok, was_err); // exactly one must be true
    assert_eq!(was_ok || was_err, true);

    // Try setting again — this must fail since global recorder is set at most once.
    let recorder2 = TestRecorder::new();
    let result2 = set_global_recorder(recorder2);
    if was_ok {
        // First call succeeded, second must fail.
        assert!(result2.is_err());
        assert!(!result2.is_ok());
    } else {
        // First call failed (already set by another test), second also fails.
        assert!(result2.is_err());
        assert!(!result2.is_ok());
    }

    // Verify we can still use with_recorder after setting global
...
fn test_with_local_recorder_basic_workflow() {
    ...
    let result = with_local_recorder(&recorder, || {
        // Inside the closure, the local recorder is active.
        // Use with_recorder to interact with it.
        with_recorder(|rec| {
    ...
    // After the closure, the local recorder is no longer active.
    let ops_after = ops.load(Ordering::SeqCst);

    // Call with_recorder outside local scope — should not hit our counting recorder
    with_recorder(|rec| {
    ...
    // ops should not have changed since we're outside the local recorder scope
...
fn test_with_local_recorder_multiple_operations() {
    ...
    // 3 register calls + 2 describe calls = 5
...
fn test_with_local_recorder_nested_scopes() {
    ...
    let result = with_local_recorder(&recorder_outer, || {
        ...
        // After inner scope, outer should be active again
        with_recorder(|rec| {
    ...
    // Outer recorder: 1 register_counter + 1 register_gauge = 2
    assert_eq!(ops_outer.load(Ordering::SeqCst), 2);
    // Inner recorder: 1 register_counter + 1 register_gauge = 2
    assert_eq!(ops_inner.load(Ordering::SeqCst), 2);
}

#[test]
fn test_with_recorder_return_values() {
    // with_recorder should pass through arbitrary return types
...
fn test_with_local_recorder_does_not_leak() {
    ...
    // Verify the recorder is no longer active by checking ops doesn't change
    with_recorder(|rec| {
        let metadata = Metadata::new(module_path!(), metrics::Level::INFO, None);
    ...
    assert_eq!(final_count, 10); // unchanged
}
...
fn test_with_local_recorder_describe_operations() {
    ...
    // Verify describe with different units
    with_local_recorder(&recorder, || {
        with_recorder(|rec| {
            rec.describe_histogram(
