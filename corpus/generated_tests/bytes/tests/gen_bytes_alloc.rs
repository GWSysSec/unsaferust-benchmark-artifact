//! Substitutes for `test_bytes_odd_alloc.rs` and `test_bytes_vec_alloc.rs`,
//! which were disabled because they install a `#[global_allocator]` that
//! clashes with the `unsafe_perf` heap_tracker hook. These hand-written
//! tests exercise the same `Vec<u8>` -> `Bytes` vtable paths (promotable
//! even/odd vtables, promotion on clone, drop ordering) under the system
//! allocator so the bench-relevant code paths remain covered.
