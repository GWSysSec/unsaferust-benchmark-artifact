//! Balanced unsafe-oriented test for `msgpack-rust` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! The crate's own unsafe (a transmute in the Raw serializer) executes only a
//! handful of instructions, dwarfed by the safe (de)serialization path.
...
const SEED: u8 = 0x6e;
const PASSES: u64 = 53;         // load/store passes over the unsafe buffers (RQ1/RQ3)
const SAFE_ITERS: u64 = 11735852; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1303; // safe heap allocations: memory dilution (RQ2)
