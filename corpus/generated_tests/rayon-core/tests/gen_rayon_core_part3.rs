...
fn join_context_returns_values_and_inspects_ctx() {
    ...
    let (ra, rb) = join_context(
        move |ctx: FnContext| {
            // Reading migrated() must not panic; record that we visited
    ...
    // Nested join_context to push work that may migrate.
...
fn scope_nested_spawning_accumulates() {
    ...
    // scope returns the value produced by op
...
fn in_place_scope_runs_op_on_caller_thread() {
    ...
    // Sum specific values via spawned tasks
    ...
    // Empty op returns its value through
...
fn in_place_scope_fifo_runs_op_on_caller_thread() {
    ...
    // Second invocation accumulates again into a fresh counter
    let count = AtomicUsize::new(0);
...
fn broadcast_yields_one_value_per_worker() {
    ...
    // current_thread_index inside the broadcast should equal ctx.index()
    ...
    // Broadcast can return computed values
...
fn spawn_broadcast_runs_on_each_worker() {
    ...
    // Submit two more broadcasts; each must hit every worker exactly once.
    ...
    // Indices observed inside spawn_broadcast cover 0..n
...
fn spawn_fifo_completes_all_submitted_jobs() {
    ...
    // Fire a second wave; counters must continue to accumulate.
    for _ in 0..5 {
        let c = count.clone();
        spawn_fifo(move || {
