...
fn config_thread_name_and_start_handler() {
    ...
    // start_handler fired once per worker.
    ...
    // thread_name applied to all workers.
...
fn config_panic_handler_fires_and_pool_recovers() {
    ...
    // Pool still functional: run a scope and verify the sum.
    ...
    // exit_handler must NOT have fired — global pool workers are still alive.
    assert_eq!(s.exit_count.load(Ordering::SeqCst), 0);

    // No extra panic_handler invocations after recovery work.
...
fn config_start_handler_does_not_refire() {
    ...
    // Drive more work to confirm start_handler is not invoked again.
