...
fn join_recursive_parallel_sum_is_exact() {
    ...
    // Mixed types, return order preserved
...
fn join_context_runs_both_closures_and_exposes_migrated() {
    ...
    // migrated() must have written 0 or 1 to both observers (not the sentinel 2)
...
fn scope_nested_inner_spawns_all_complete() {
    ...
    // 5 outer * 1 + 5 outer * 3 inner * 10 = 5 + 150 = 155
...
fn broadcast_runs_on_every_worker_with_unique_indices() {
    ...
    // broadcast returning a different result type
...
fn spawn_broadcast_runs_on_every_worker() {
    // Determine worker count via broadcast (which yields one item per worker).
