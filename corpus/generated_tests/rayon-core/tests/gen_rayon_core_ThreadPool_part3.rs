...
fn threadpool_install_returns_value_and_runs_on_pool() {
    ...
    let result: i64 = pool.install(move || {
        // Inside the pool, current_thread_index should be Some(_).
        ...
        // current_num_threads inside install must reflect this pool.
    ...
    // Outside install: not necessarily on a worker.
...
fn threadpool_broadcast_visits_every_worker_exactly_once() {
    ...
    let results: Vec<usize> = pool.broadcast(move |ctx: BroadcastContext<'_>| {
        let idx = ctx.index();
        // Record under timed try_lock loop.
...
fn threadpool_in_place_scope_executes_outside_pool() {
    ...
    // Caller is not on the pool, so in_place_scope from outside runs the scope
    // logic on the calling thread but spawns to the pool.
...
fn threadpool_spawn_broadcast_visits_every_worker() {
    ...
    // Wait for all 4 workers to record their visit.
    let start = Instant::now();
...
fn threadpool_combined_workflow() {
    ...
    // Step 1: install + nested join produces a sum.
    ...
    // Step 2: broadcast confirms worker count and indices.
    ...
    // Step 3: scope spawns work that uses the prior result.
    ...
    // Step 4: spawn_fifo + wait completes outside install.
