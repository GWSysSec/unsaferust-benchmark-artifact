...
fn scope_fifo_spawn_fifo_completes_all_tasks() {
    ...
    // Post-state: scope_fifo blocks until all spawned tasks complete.
    ...
    // All values 0..n_tasks must appear exactly once, regardless of order.
    let mut sorted = final_record.clone();
    sorted.sort();
    let expected: Vec<usize> = (0..n_tasks).collect();
    assert_eq!(sorted, expected);

    // The set must contain min and max boundary values.
...
fn scope_fifo_spawn_fifo_nested_recursive() {
    ...
    // Binary tree of depth 4: 1 + 2 + 4 + 8 + 16 = 31 total node visits.
    // Leaves at depth 0 = 16.
