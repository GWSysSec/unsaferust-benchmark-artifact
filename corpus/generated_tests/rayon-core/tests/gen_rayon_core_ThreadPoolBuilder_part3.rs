...
fn test_builder_thread_name_start_exit_handlers() {
...

    {
        ...
        // Drive work that touches every worker so thread_name is observed.
        ...
        // Wait for all start_handlers to fire.
        ...
        // thread_name closure output must propagate to thread name.
        ...
        // exit_handler must NOT fire while pool is alive.
        assert_eq!(exit_count.load(Ordering::SeqCst), 0);
        assert_eq!(exit_indices.lock().unwrap().len(), 0);
    } // pool dropped here
...
fn test_builder_panic_handler_isolates_panics() {
    ...
    // Fire-and-forget panicking spawns via the installed pool.
    ...
    // Pool remains usable after panics.
