...
fn test_encode_from_utf8_to_vec_unmappable_emoji_in_windows1252() {
    ...
    let input = "Hi 😀!";
    assert_eq!(input.len(), 8); // "Hi " (3) + emoji (4) + "!" (1)
    ...
    assert_eq!(read, 7); // "Hi " (3 bytes) + 4 bytes of emoji consumed
    output.truncate(written);
    assert_eq!(output.len(), 3);
    assert_eq!(&output[..], b"Hi ");
    assert_ne!(output.len(), input.len());

    // Resume with remainder
...
fn test_encode_from_utf8_to_vec_gbk_chinese_chars() {
    ...
    let input = "中文ABC";
    assert_eq!(input.len(), 9); // 3+3+1+1+1
    ...
    assert_eq!(read, 9);
    output.truncate(written);
    // GBK: each Chinese char is 2 bytes, each ASCII is 1 byte
    assert_eq!(output.len(), 7);
    // ASCII portion preserved as-is at the tail
    assert_eq!(output[4], b'A');
    assert_eq!(output[5], b'B');
    assert_eq!(output[6], b'C');
    // Chinese bytes are non-ASCII
...
fn test_encode_from_utf8_to_vec_utf8_passthrough_includes_astral() {
    ...
    assert_eq!(output[0], b'H');
    assert_ne!(output[7], 0); // first byte of multi-byte CJK sequence
}
...
fn test_max_buffer_length_from_utf16_windows1252_monotonic() {
    ...
    // For a single-byte encoding, we need at least 1 byte per UTF-16 code unit
    assert!(v1 >= 1);
    assert!(v10 >= 10);
    assert!(v100 >= 100);
    assert!(v1000 >= 1000);

    // Monotonic non-decreasing with respect to input length
    assert!(v1 >= v0);
    assert!(v10 >= v1);
    assert!(v100 >= v10);
    assert!(v1000 >= v100);
...
fn test_max_buffer_length_from_utf16_sizing_drives_actual_encode() {
    // Verify that the advertised capacity is actually enough to hold a real
    // encoding result when no unmappables occur.
    let encoding = Encoding::for_label(b"windows-1252").expect("windows-1252 exists");
    let mut encoder = encoding.new_encoder();
    ...
    // Now feed equivalent UTF-8 data and encode; the resulting byte length must
    // not exceed `needed` since there are no unmappables.
...
fn test_encode_from_utf8_to_vec_multiple_unmappables_sequential() {
    ...
    // Each CJK char is unmappable in windows-1252.
    let input = "A中B文C";
    let mut output: Vec<u8> = Vec::new();

    let total_bytes = input.len();
    assert_eq!(total_bytes, 9); // 1+3+1+3+1

    // First call hits the first unmappable char.
    let max1 = encoder
        .max_buffer_length_from_utf8_without_replacement(input.len())
        .expect("reasonable size");
    let mut tmp1: Vec<u8> = vec![0u8; max1];
    ...
    assert_eq!(c2, '文');
    assert_eq!(read2, 4); // 'B' (1) + '文' (3)
    output.extend_from_slice(&tmp2[..written2]);
    assert_eq!(output.len(), 2);
    assert_eq!(output[1], b'B');

    // Finish with the trailing 'C'.
