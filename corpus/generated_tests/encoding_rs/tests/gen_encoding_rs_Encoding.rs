...
fn test_partial_and_utf16_str_conversions() {
    let latin1: &[u8] = &[0x48, 0xE9, 0x6C, 0x6C]; // "Héll" in Latin-1, 4 bytes

    // convert_latin1_to_str requires dst.len() >= src.len() * 2
    let mut big = vec![0u8; latin1.len() * 2]; // 4 * 2 = 8
    ...
    let u16_data: Vec<u16> = "Hé".encode_utf16().collect(); // 2 u16 units
    let mut str_dst = vec![0u8; u16_data.len() * 3]; // 2 * 3 = 6
    ...
    // convert_latin1_to_str_partial with a small destination
    ...
    // convert_utf16_to_str with longer data
    ...
    // convert_latin1_to_utf8_partial with small raw buffer
    ...
    // convert_latin1_to_str with exact-sized destination
    let exact_dst_len = latin1.len() * 2; // 4 * 2 = 8
