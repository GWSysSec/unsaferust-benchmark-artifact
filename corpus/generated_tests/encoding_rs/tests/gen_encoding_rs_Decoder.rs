...
fn make_str_buf(buf: &mut [u8]) -> &mut str {
    // Zero-init bytes form valid UTF-8 (NUL chars).
...
fn test_decode_to_str_output_full() {
    ...
    let src = b"abcdefghij"; // 10 bytes ASCII
    let mut backing = [0u8; 4]; // intentionally tiny
    let dst = make_str_buf(&mut backing);
    ...
    // Drain the rest into a larger buffer.
...
fn test_decode_to_str_with_replacement_on_invalid_utf8() {
    ...
    // 0xFF is never valid UTF-8 — should produce U+FFFD.
...
fn test_decode_to_str_without_replacement_success() {
    ...
    let src = "héllo".as_bytes(); // valid UTF-8 with one 2-byte char
    let mut backing = [0u8; 64];
    let dst = make_str_buf(&mut backing);
...
fn test_decode_to_str_without_replacement_malformed_and_output_full() {
    ...
    let src = &[b'X', 0xC0, 0xC0, b'Y']; // invalid UTF-8 in the middle
    let mut backing = [0u8; 64];
    let dst = make_str_buf(&mut backing);
    ...
    // OutputFull branch — fresh decoder, tiny buffer.
...
fn test_decode_to_str_multibyte_split_across_calls() {
    // Feed a multi-byte UTF-8 character split across two calls; the decoder
    // should buffer the partial sequence and emit nothing for the first chunk,
    // then complete it on the second.
    let enc = Encoding::for_label(b"utf-8").unwrap();
    let mut decoder = enc.new_decoder();
    ...
    assert_eq!(read2, 2);
    assert_eq!(errs2, false);
    assert!(written2 >= 3); // "é" (2 bytes) + "!" (1 byte)
