...
fn test_token_serialization_type_nothing_equality() {
    ...
    // All nothing() values should be equal to each other
    assert_eq!(a, b);
    assert_eq!(b, c);
    assert_eq!(a, c);
    assert_eq!(a, TokenSerializationType::nothing());

    // TokenSerializationType is Copy; original should remain usable
    let d = a;
    let e = b;
    assert_eq!(d, a);
    assert_eq!(e, b);
...
fn test_token_serialization_type_set_if_nothing_idempotent() {
    ...
    // Pre-state: x is nothing. set_if_nothing(nothing) => remains nothing.
    x.set_if_nothing(TokenSerializationType::nothing());
    assert_eq!(x, baseline);

    // Call a second time: same result.
    x.set_if_nothing(TokenSerializationType::nothing());
    assert_eq!(x, baseline);

    // Many repeated calls must not mutate it into something else.
    for _ in 0..50 {
        x.set_if_nothing(TokenSerializationType::nothing());
    }
    assert_eq!(x, baseline);

    // Independent instance, same behavior.
    let mut y = TokenSerializationType::nothing();
    y.set_if_nothing(TokenSerializationType::nothing());
    assert_eq!(y, x);
    assert_eq!(y, baseline);

    // Bulk: many independent instances all converge to the nothing value.
...
fn test_token_serialization_type_alongside_parser() {
    // Realistic multi-step workflow: construct a parser, use nothing()
    // at various points, ensure values remain stable across parser lifetime.
...
fn test_parse_nth_multi_case_workflow() {
    // Exercise parse_nth on many inputs, combined with TokenSerializationType
    // checks to keep the uncovered API in a realistic context.
    ...
    for (src, expected_a, expected_b) in &cases {
        ...
        // Interleave set_if_nothing during iteration; must not diverge.
        tracker.set_if_nothing(TokenSerializationType::nothing());
        assert_eq!(tracker, baseline);
    }

    // Bad input should produce an error, not a panic.
...
fn test_parse_important_workflow() {
    // Valid !important, canonical form.
    let mut input = ParserInput::new("!important");
    let mut parser = Parser::new(&mut input);
    assert!(parse_important(&mut parser).is_ok());

    // CSS identifiers are ASCII-case-insensitive for known keywords.
    ...
    // Target-API: TokenSerializationType before and after parsing.
