    ...
    loom::model(|| {
        unsafe {
            ...
            *ptr2 = 0xBB;
            ...
            // Verify writes didn't interfere
            assert_eq!(*ptr1, 0xAA);
            assert_eq!(*ptr2, 0xBB);
...
fn alloc_zeroed_large_block() {
    loom::model(|| {
        unsafe {
            ...
            // Check zeroed at various offsets
            assert_eq!(*ptr.add(0), 0u8);
    ...
    loom::model(|| {
        unsafe {
            ...
            *ptr1 = 0x99;
            ...
            // Second allocation with same layout
            let ptr2 = alloc_zeroed(layout);
            assert!(!ptr2.is_null());
            // alloc_zeroed guarantees zeros regardless of reuse
