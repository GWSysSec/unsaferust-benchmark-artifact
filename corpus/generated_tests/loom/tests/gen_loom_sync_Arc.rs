...
fn arc_into_raw_and_as_ptr() {
    loom::model(|| {
        ...
        // into_raw does NOT decrement the strong count; it consumes the Arc
        // without dropping it. So strong count is still 2 (one for `a`, one
        // for the raw pointer we now own).
        assert_eq!(Arc::strong_count(&a), 2);
...
fn arc_increment_decrement_strong_count() {
    loom::model(|| {
        ...
        // strong count is still 1 (into_raw doesn't decrement)
...
fn arc_into_raw_roundtrip() {
    loom::model(|| {
        ...
        let raw = Arc::into_raw(b);
        // `a` still alive, and the raw pointer owns one reference too,
        // so strong count should still be 2.
        assert_eq!(Arc::strong_count(&a), 2);
...
fn arc_pin_with_clone_and_strong_count() {
    loom::model(|| {
        ...
        // Pin<Arc<T>> can be cloned since Arc<T>: Clone
        let pinned2 = pinned.clone();
        let inner2 = pinned2.as_ref().get_ref();
        assert_eq!(*inner2, 88u32);

        // Both point to same allocation
        // We can't easily use Arc::ptr_eq on Pin<Arc<T>> directly,
        // but we can check the values are consistent
