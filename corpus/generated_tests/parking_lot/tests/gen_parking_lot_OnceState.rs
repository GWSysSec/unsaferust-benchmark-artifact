...
fn once_state_done_initial_and_after_completion() {
    ...
    // Second call_once must not execute the closure
    once.call_once(|| {
...
fn once_state_done_receives_false_inside_first_force_call() {
    ...
    let inside_done = AtomicBool::new(true); // sentinel: should be flipped to false
    ...
    // A second force call on a done Once must not invoke the closure.
...
fn once_state_done_stable_after_many_redundant_calls() {
    ...
    // Snapshots of the state taken at different times must agree on done().
    let s_a = once.state();
    let s_b = once.state();
    let s_c = once.state();
