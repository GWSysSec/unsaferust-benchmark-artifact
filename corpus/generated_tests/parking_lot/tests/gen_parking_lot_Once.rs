...
fn test_call_once_force_after_poison() {
    ...
    // Poison the Once via a panic in the initializer.
    ...
    // call_once on a poisoned Once should panic.
    ...
    // call_once_force should run despite poisoning.
    ...
    // After successful force, further calls do nothing.
...
fn test_call_once_force_clean_state() {
    ...
    // Subsequent call_once_force should not re-run.
    once.call_once_force(|_| {
        runs += 1;
    });
    assert_eq!(runs, 1);
...
fn test_multi_step_initialization_workflow() {
    ...
    // Stage 1: many threads racing to initialize.
    let mut handles = Vec::new();
    for _ in 0..3 {
        let once_c = Arc::clone(&once);
    ...
    // Stage 2: ensure call_once_force after success doesn't reinitialize.
    once.call_once_force(|state: OnceState| {
