...
fn test_ssl_ref_state_and_settings() {
    ...
    // set_connect_state then set_accept_state — both should be callable
    ssl.set_connect_state();
    ssl.set_accept_state();
    // verify_mode persists across state changes
    ...
    // verify_result on an un-handshaken Ssl: should be OK (no peer cert seen yet)
    let vr = ssl.verify_result();
    assert_eq!(vr, openssl::x509::X509VerifyResult::OK);

    // current_cipher / peer_cert_chain / version2 should be None pre-handshake
    assert!(ssl.current_cipher().is_none());
    assert!(ssl.peer_cert_chain().is_none());
    // After set_accept_state, version2() may return Some depending on OpenSSL version.
...
fn test_full_handshake_introspection() {
    ...
    // current_cipher should be Some after handshake
    ...
    // peer_cert_chain present
    let chain = s.peer_cert_chain().expect("peer chain present");
    assert!(chain.len() >= 1);

    // verified_chain: may be None when verify is NONE; just ensure call works
    let _vc = s.verified_chain();

    // verify_result — with NONE verify mode and a self-signed cert, the result
    // may be "self-signed certificate" (code 18). Just verify the call works.
    let _vr = s.verify_result();

    // export_keying_material_early — typically fails for non-early-data scenarios
