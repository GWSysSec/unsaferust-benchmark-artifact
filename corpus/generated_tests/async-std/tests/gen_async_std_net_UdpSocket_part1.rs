...
fn udp_send_recv_from_roundtrip() {
    task::block_on(async {
        ...
        // Bytes past the received length must not be overwritten.
        assert_eq!(buf[n], 0u8);
    });
}
...
fn udp_peek_from_does_not_consume_datagram() {
    task::block_on(async {
        ...
        // Peek a second time — datagram should still be queued.
        ...
        // Finally consume it with recv_from.
...
fn udp_broadcast_getter_setter_roundtrip() {
    task::block_on(async {
        ...
        // Starting state must be a valid bool.
        ...
        // Flip one more time and confirm it sticks.
...
fn udp_join_multicast_v4_state_preserved() {
    task::block_on(async {
        ...
        // Configure options before joining.
        ...
        // All previously configured socket options must survive the join.
        assert_eq!(sock.multicast_loop_v4().unwrap(), true);
        assert_eq!(sock.multicast_ttl_v4().unwrap(), 4);
        assert_eq!(sock.ttl().unwrap(), 33);

        // Socket is still usable — local_addr must be bound.
...
fn udp_multicast_loop_v6_roundtrip() {
    task::block_on(async {
        ...
        // Flip one more time to ensure no stuck state.
...
fn udp_join_multicast_v6_then_configure() {
    task::block_on(async {
        ...
        // Interface-local, all-nodes-like scope test group.
        let group = Ipv6Addr::new(0xff02, 0, 0, 0, 0, 0, 0, 0x1234);

        // Joining with interface 0 means "any / default". In some
        // container environments this can fail; assert that either it
        // succeeds cleanly or the error is a plain io::Error the caller
        // could handle.
        let join_result = sock.join_multicast_v6(&group, 0);
        let joined = join_result.is_ok();
        if let Err(ref e) = join_result {
            // Ensure it's a real io::Error with a meaningful kind, not a panic.
            let _ = e.kind();
        }
        assert!(joined || join_result.is_err());

        // Subsequent options still work regardless of join outcome.
        sock.set_multicast_loop_v6(false)
        ...
        // And the socket can still send (to itself, nothing receiving).
