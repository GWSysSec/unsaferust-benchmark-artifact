...
fn test_copy_repeat_to_sink() {
    block_on(async {
        // limit repeat with take via Read trait
...
fn test_stdout_stderr_handles() {
    block_on(async {
        ...
        // Writing empty should produce 0 written or succeed
...
fn test_stdin_handle_exists() {
    let _stdin = io::stdin();
    // Just ensure constructing succeeds; use a trivial probe
    let s1 = io::sink();
    let s2 = io::sink();
    drop(s1);
    drop(s2);
