...
fn test_filetime_zero_and_now_basics() {
    ...
    // now() must produce a plausible modern timestamp
...
fn test_set_file_mtime_to_epoch_zero() {
    ...
    // Pre-state: mtime should be near now
...
fn test_set_file_times_both_and_atime_override() {
    ...
    // Set both atime and mtime to zero
    ...
    // Now override only atime with now()
    let now = FileTime::now();
    ...
    // atime moved to ~now, mtime still at epoch
...
fn test_from_creation_time_on_fresh_file() {
    ...
    match maybe_ct {
        Some(ct) => {
            ...
            // Creation time must be a plausible modern time
            assert!(ct_s > 1_577_836_800, "ctime={}", ct_s);
            // Created at or before "now" (allow small clock skew)
            assert!(ct_s <= now.unix_seconds() + 2);
            // Within the last minute since we just created it
            assert!(ct_s >= before_now - 60);
            assert_ne!(ct, zero);
            assert_ne!(ct_s, 0);
        }
        None => {
            // Filesystem didn't expose birth time; still validate invariants
...
fn test_set_file_handle_times_selective() {
    ...
    // First, reset both to epoch via handle API
    ...
    // Now advance only mtime, leave atime as None (implementation preserves prior)
    let now = FileTime::now();
    let now_s = now.unix_seconds();
    assert!(now_s > 1_577_836_800);
...
fn test_set_symlink_file_times_does_not_touch_target() {
    ...
    #[cfg(unix)]
    {
        ...
        // Capture target mtime before symlink-time mutation
        ...
        // Set symlink's own timestamps to zero
        ...
        // Target mtime must be unchanged
