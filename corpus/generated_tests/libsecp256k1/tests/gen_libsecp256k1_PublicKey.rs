...
fn test_tweak_add_assign_basic() {
    ...
    // Verify the tweaked key is still a valid public key by signing and verifying
    let message = Message::parse(&MESSAGE_BYTES);
    // The tweaked public key should not verify a signature made with the original secret key
    ...
    // Applying the same tweak again should produce a different result
...
fn test_tweak_add_assign_with_context_matches_without_context() {
    ...
    // Verify both are valid by recovering from a signature
    let message = Message::parse(&MESSAGE_BYTES);
    ...
    // Both should give the same verification result (both should fail since key was tweaked)
...
fn test_tweak_mul_assign_basic() {
    ...
    // Mul tweak should differ from add tweak
    ...
    // Verify the tweaked key is a valid point (can be serialized/deserialized)
...
fn test_tweak_mul_assign_with_context_matches_without_context() {
    ...
    // Compressed forms should also match
...
fn test_tweak_mul_assign_deterministic() {
    ...
    // Applying mul tweak with identity-like scalar (1) should keep the key the same
    let one_bytes: [u8; 32] = [
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
fn test_tweak_add_assign_commutativity_of_tweaks() {
    // Adding tweak A then tweak B should give same result as adding (conceptually) B then A
    // because point addition is commutative: P + aG + bG == P + bG + aG
    let sk1 = SecretKey::parse(&SECRET_KEY_1).unwrap();
    let sk2 = SecretKey::parse(&SECRET_KEY_2).unwrap();
    let sk3 = SecretKey::parse(&SECRET_KEY_3).unwrap();
    ...
    // But the intermediate states should differ
...
fn test_tweak_add_then_sign_and_recover() {
    ...
    // The tweaked public key should correspond to the secret key (sk1 + sk2) mod n
    // We can verify this indirectly: sign with sk1, verify with original (should pass)
    let message = Message::parse(&MESSAGE_BYTES);
    ...
    // Recover should give back the original public key
...
fn test_tweak_mul_assign_associativity() {
    // (P * a) * b should equal (P * b) * a because scalar multiplication is commutative
    let sk1 = SecretKey::parse(&SECRET_KEY_1).unwrap();
    let sk2 = SecretKey::parse(&SECRET_KEY_2).unwrap();
    let sk3 = SecretKey::parse(&SECRET_KEY_3).unwrap();
    ...
    // Intermediate states should differ
...
fn test_tweak_add_and_mul_combined_workflow() {
    ...
    // Start with a public key, apply add tweak, then mul tweak
    let mut pubkey = PublicKey::from_secret_key(&sk1);
    ...
    // Reverse order should give different result (add then mul != mul then add)
    ...
    // Both results should be valid public keys (parseable compressed form)
...
fn test_tweak_add_assign_with_different_base_keys() {
    ...
    // Tweaking different base keys with the same tweak should give different results
    ...
    // Add and mul with same tweak on same base should differ
...
fn test_tweak_mul_assign_with_context_explicit_context() {
    ...
    // Applying the tweak three times
    ...
    // Verify the result is a valid point
...
fn test_tweak_add_assign_preserves_point_validity_across_multiple_tweaks() {
    ...
    // Final key should still be a valid compressed point
