...
/// Tests for `cc::windows_registry::find` on Linux.
/// Since we're on Linux, these functions will return None for MSVC tools,
/// but we can still exercise the API surface and verify behavior.
#[test]
fn test_windows_registry_find_returns_none_on_linux() {
    // On Linux, find should return None for all MSVC-related tools
...
fn test_windows_registry_find_tool_returns_none_on_linux() {
    // On Linux, find_tool should return None for all MSVC-related tools
...
fn test_windows_registry_find_vs_version_on_linux() {
    // On Linux, find_vs_version should return an Err since no VS is installed
    let result = windows_registry::find_vs_version();

    // On Linux this should be an error
    ...
    // Verify we can call it multiple times consistently
    ...
    // Both error messages should be the same (deterministic behavior)
    assert_eq!(err_msg, err_msg2, "Error messages should be consistent across calls");

    // Verify the error message contains some useful information
    assert!(err_msg.len() > 3, "Error message should be descriptive, got: {}", err_msg);

    // The error should be a String we can clone and compare
...
fn test_windows_registry_find_various_architectures() {
    // Exercise find with various architecture strings to ensure no panics
    let architectures = [
        "x86",
        "x86_64",
        "aarch64",
...
fn test_windows_registry_find_tool_consistency() {
    // Verify that calling find_tool multiple times with the same args gives consistent results
    let arch = "x86_64";
    ...
    // Also verify find is consistent
...
fn test_windows_registry_find_with_unusual_inputs() {
    // Test with unusual but valid string inputs to ensure no panics
