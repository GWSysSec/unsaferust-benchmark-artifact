...
fn clang_cl_tool_reports_driver_kind_and_environment_strings() {
    ...
    // `clang-cl` has its own tool family in cc: it is clang-cl-like and
    // MSVC-like, but not reported by `is_like_clang()`, which is for the GNU
    // style clang driver.
    ...
    // For a compiler configured directly through Build::compiler, cc_env()
    // does not duplicate the compiler path. The configured command is still
    // available through Tool::path() and Tool::to_command().
    assert!(
        cc_env.is_empty(),
...
fn gnu_like_tool_environment_strings_track_configured_flags() {
    ...
    // `cc_env()` contains compiler fragments parsed from an environment-style
    // compiler definition. A compiler set directly with Build::compiler is
    // represented by Tool::path(), not repeated here.
    assert!(
        cc_env.is_empty(),
        "directly configured compiler should not add CC env fragments: {:?}",
        cc_env
