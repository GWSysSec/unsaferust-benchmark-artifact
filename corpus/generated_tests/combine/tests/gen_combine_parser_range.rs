...
fn test_take_while_digits() {
    ...
    // empty match should still succeed
...
fn test_take_fn_take() {
    ...
    // not found - should fail because more input expected (NotFound on non-partial)
    let res2: Result<(&str, &str), _> = p.parse("abc");
    // For non-partial input, NotFound typically means need more input -> error
