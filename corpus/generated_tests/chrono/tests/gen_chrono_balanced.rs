//! Balanced unsafe-oriented test for `chrono` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! On a Linux --release test build chrono reaches zero executable unsafe (its
//! few unsafe sites are libc time conversions not hit by the in-tree tests).
...
const SEED: u8 = 0x3c;
const PASSES: u64 = 1324;         // load/store passes over the unsafe buffers (RQ1/RQ3)
