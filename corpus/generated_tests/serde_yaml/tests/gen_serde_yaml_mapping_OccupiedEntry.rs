...
fn occupied_entry_get_reads_value() {
    ...
    // Mapping unchanged after read-only entry access.
    assert_eq!(m.len(), 2);
...
fn occupied_entry_get_mut_mutates_value() {
    ...
    // Post-state: mutation should be visible through the Mapping.
    assert_eq!(m.len(), 2);
    assert_eq!(
        m.get(&Value::String("counter".to_string())),
    ...
    // Replace a different value via get_mut to a completely different variant.
...
fn occupied_entry_remove_entry_returns_pair_and_shrinks() {
    ...
    // Re-inserting after remove_entry should make it Occupied again.
...
fn occupied_entry_workflow_after_yaml_parse() {
    ...
    // Mutate via get_mut, then remove a different key via remove_entry.
