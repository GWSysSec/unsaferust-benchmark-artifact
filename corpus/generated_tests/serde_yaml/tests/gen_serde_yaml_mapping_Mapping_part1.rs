...
fn test_with_capacity_reserve_shrink_cycle() {
    ...
    // reserving on already-large mapping is a no-op or grow only
...
fn test_contains_key_and_get_multiple_types() {
    ...
    // contains_key with an owned Value key (via blanket Index on &T)
...
fn test_shift_remove_preserves_order() {
    ...
    // Remaining order should be a, d, e (shift preserves order)
...
fn test_iter_mut_bulk_update() {
    ...
    // Double every value through iter_mut
    ...
    // iter_mut length unchanged after mutations
...
fn test_clear_then_reuse_via_parse() {
    ...
    // clear does not reduce capacity below pre-clear value
    ...
    // A freshly-built mapping is independent
...
fn test_remove_entry_vs_shift_remove_semantics() {
    // Confirm both removal families work independently on separate maps.
