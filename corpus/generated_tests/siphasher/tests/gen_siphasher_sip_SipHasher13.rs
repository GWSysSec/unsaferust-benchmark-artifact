...
fn test_siphasher13_default_keys_are_zero() {
    ...
    // `new()` must construct a hasher with both keys zeroed.
    assert_eq!(k0, 0u64);
    assert_eq!(k1, 0u64);

    // `key()` must return a fixed-size 16-byte array.
    ...
    // `key()` must be the little-endian concatenation of the two u64s from `keys()`.
    let mut expected = [0u8; 16];
    expected[0..8].copy_from_slice(&k0.to_le_bytes());
    expected[8..16].copy_from_slice(&k1.to_le_bytes());
    assert_eq!(key_bytes, expected);

    // Independently-constructed hashers must agree on default keys byte-for-byte.
...
fn test_siphasher13_keys_stable_across_writes() {
    ...
    // Keys must be invariant under a single `write` call.
    ...
    // Keys must still be invariant after a multi-step typed write sequence.
    ...
    // Finishing the hash must leave keys untouched as well.
...
fn test_siphasher13_key_byte_layout_multistep() {
    ...
    for payload in payloads.iter() {
        ...
        // Pre-write state: both accessors agree on default-zero keys.
        let (k0, k1) = hasher.keys();
        let kb = hasher.key();
        assert_eq!(k0, 0u64);
        assert_eq!(k1, 0u64);
        ...
        // keys() and key() must be mutually derivable (LE layout).
        ...
        // Post-write: keys remain stable regardless of payload length.
    ...
    // Differing inputs must produce differing SipHash-1-3 outputs in this set.
    ...
    // Determinism: same input + same default keys => same digest.
...
fn test_siphasher13_key_accessors_consistency_after_reuse() {
    // Reuse a single hasher across multiple "rounds" of write+finish and
    // confirm that keys()/key() report the identical 16-byte seed every time.
    let mut hasher = SipHasher13::new();
    ...
    for msg in messages.iter() {
        ...
        // Every round must yield the same key material.
    ...
    // Final sanity: the byte-form accessor still matches a freshly derived LE encoding.
