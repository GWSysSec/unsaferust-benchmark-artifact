...
fn test_sip128_keys_default_zero() {
    ...
    // Cross-check: key bytes must decode (little-endian) back to keys.
    let mut lo = [0u8; 8];
    let mut hi = [0u8; 8];
...
fn test_sip128_keys_invariant_over_writes() {
    ...
    // Write arbitrary chunks — keys must remain unchanged.
    ...
    // After all writes, produce a digest and verify it is not the trivial zero.
...
fn test_sip128_keys_multi_instance_identical() {
    ...
    // All default-constructed hashers share the same (zero) keys.
    ...
    assert_eq!(b2, b3);
    assert_eq!(b1, [0u8; 16]);
    // Ensure the 16-byte key length is as advertised.
    assert_eq!(b1.len(), 16);
}
...
fn test_sip128_key_bytes_and_keys_consistent() {
    ...
    // Reassemble u64 keys from the little-endian byte layout.
    let mut lo = [0u8; 8];
    let mut hi = [0u8; 8];
    ...
    // Every byte must be zero in the default key.
...
fn test_sip128_full_workflow_determinism() {
    ...
    // Multi-step writes simulating a real hashing workflow.
    ...
    // Key identity preserved throughout.
    ...
    // Second hasher, same operations, must produce identical digest and keys.
    ...
    // Keys still unchanged on the second hasher after writing.
...
fn test_sip128_keys_key_roundtrip_large_input() {
    ...
    // Feed a sizeable buffer (1 MiB) to exercise the streaming path.
    let buf = vec![0xA5u8; 1024 * 1024];
    hasher.write(&buf);

    // Keys must remain constant.
    ...
    // Finish and check determinism with a fresh hasher.
    ...
    // Empty-input hash of a fresh hasher must differ from the large-input hash.
    let empty_hasher = SipHasher::new();
    let empty_digest = empty_hasher.finish();
    assert_ne!(digest1, empty_digest);
    // Keys on the untouched hasher still zero.
