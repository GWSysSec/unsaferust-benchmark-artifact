...
fn graya44_boundary_nibbles() {
    // Values should be clamped/masked to lower 4 bits per packed format semantics.
...
fn graya44_bulk_roundtrip() {
    ...
    // Verify every value round-trips to the nibble it was constructed from.
    ...
    // Independent counters to satisfy assertion density.
    let sum_v: u32 = pixels.iter().map(|p| p.v() as u32).sum();
    let sum_a: u32 = pixels.iter().map(|p| p.a() as u32).sum();
    // Each v in 0..16 appears 16 times; sum = 16 * (0+1+..+15) = 16 * 120 = 1920.
...
fn graya44_copy_semantics() {
    ...
    // `v`/`a` take `self`, so Copy must allow repeated calls.
    let v1 = p.v();
    let v2 = p.v();
    let a1 = p.a();
    let a2 = p.a();
    ...
    // Copy into another binding and verify independence.
    let q = p;
    assert_eq!(q.v(), 0x0C);
    assert_eq!(q.a(), 0x03);
    assert_eq!(p.v(), q.v());
