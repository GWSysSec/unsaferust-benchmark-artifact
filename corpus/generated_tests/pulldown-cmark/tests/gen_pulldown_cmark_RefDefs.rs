...
// LinkDef is not directly exported from the crate root.
// Based on the API reference, RefDefs::iter() returns impl Iterator<Item = (&str, &LinkDef<'input>)>
// We need to access LinkDef through the RefDefs iterator without importing it directly.
// We'll just remove the import and work with the type inference.
...
fn refdefs_iter_basic_single_definition() {
    ...
    // Iterating twice should yield the same result (non-consuming).
    let count_second: usize = refdefs.iter().count();
    assert_eq!(count_second, 1);

    // Exhaust iterator: after count, can still iterate again.
...
fn refdefs_iter_empty_when_no_definitions() {
    ...
    // Also verify that iterating multiple times produces the same empty result.
    let again: Vec<_> = refdefs.iter().collect();
    assert_eq!(again.len(), 0);

    // next() on a fresh iterator is None.
    let mut it = refdefs.iter();
    assert!(it.next().is_none());
    assert!(it.next().is_none());

    // Confirm different markdown without defs also yields zero.
    let md2 = "# Heading\n\n- list item\n- another\n";
...
fn refdefs_iter_case_insensitive_label_normalization() {
    // Reference labels in CommonMark are matched case-insensitively.
...
"#;
    ...
    // Look up by normalized label: the iter keys should match the labels we can look up.
    // Even if normalized, we can test both defs are present with correct destinations.
    let foo_def = refdefs
        .iter()
...
fn refdefs_iter_duplicate_labels_keeps_first() {
    // Per CommonMark, if there are multiple definitions with the same label, the first takes precedence.
...
fn refdefs_iter_span_fields_are_valid() {
    ...
    // Each LinkDef has a span which is a Range<usize> within the input.
    for (label, linkdef) in refdefs.iter() {
        assert!(!label.is_empty());
        assert!(linkdef.span.start < linkdef.span.end);
        assert!(linkdef.span.end <= md.len());

        // The span should cover a portion of the input containing the destination.
...
fn refdefs_iter_many_definitions_stress() {
    // Build a moderate corpus of reference definitions and verify iter yields all of them.
    ...
    // No titles set on any of them.
    let titled: usize = refdefs.iter().filter(|(_, v)| v.title.is_some()).count();
    assert_eq!(titled, 0);

    // Count via sum to exercise iterator adapters.
...
fn refdefs_iter_after_full_parse_still_available() {
    // The parser must still expose reference_definitions after being consumed.
    // We verify by also consuming events in a separate parser instance.
...
"#;
    ...
    // Then use a fresh parser to access reference definitions.
