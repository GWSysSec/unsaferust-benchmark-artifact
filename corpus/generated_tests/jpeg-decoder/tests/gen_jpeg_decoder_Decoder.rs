...
fn test_set_color_transform_rgb_decode() {
    ...
    // First decode without setting color transform to get baseline
    let mut decoder = Decoder::new(File::open(&path).unwrap());
    let baseline_data = decoder.decode().unwrap();
    let baseline_info = decoder.info().unwrap();

    // Now decode with explicit RGB color transform
    ...
    // The image dimensions should remain the same regardless of color transform setting
    ...
    // Both should produce data of the same length since dimensions are the same
    assert_eq!(baseline_data.len(), rgb_data.len());
    // Pixel format should be consistent
...
fn test_set_color_transform_cmyk() {
    ...
    // Decode with CMYK color transform
    let mut decoder = Decoder::new(File::open(&path).unwrap());
    decoder.set_color_transform(ColorTransform::CMYK);
    let result = decoder.decode();

    // Whether it succeeds or fails, we verify behavior is consistent
    match result {
        Ok(data) => {
            ...
            // CMYK has 4 bytes per pixel
            assert_eq!(pixel_bytes, 4);
            assert!(data.len() >= 4);
        }
        Err(_) => {
            // If CMYK transform is not applicable, that's valid behavior
            // Try without transform to confirm the file is otherwise valid
            let mut decoder2 = Decoder::new(File::open(&path).unwrap());
...
fn test_set_color_transform_ycck() {
    ...
    match result_ycck {
        Ok(ycck_data) => {
            ...
            // With YCCK transform the data may differ from default
        ...
        Err(_) => {
            // YCCK might not be applicable; baseline should still work
...
fn test_set_max_decoding_buffer_size_too_small() {
    ...
    // First, find out how large the decoded buffer actually is
    let mut decoder_baseline = Decoder::new(File::open(&path).unwrap());
    let baseline_data = decoder_baseline.decode().unwrap();
    let baseline_info = decoder_baseline.info().unwrap();
    ...
    // Now set a max buffer size that is too small
    let mut decoder_limited = Decoder::new(File::open(&path).unwrap());
    decoder_limited.set_max_decoding_buffer_size(1); // 1 byte is way too small
    let result = decoder_limited.decode();

    // Decoding should fail because the buffer would exceed the limit
    assert!(result.is_err());

    // Verify the error can be formatted (exercises Error::fmt)
...
fn test_set_max_decoding_buffer_size_exact() {
    ...
    // Set max buffer size to exactly the needed size - should succeed
    ...
    // Set max buffer size to one less than needed - should fail
...
fn test_set_max_decoding_buffer_size_generous() {
    ...
    // Set a very generous buffer size
    let mut decoder = Decoder::new(File::open(&path).unwrap());
    decoder.set_max_decoding_buffer_size(100 * 1024 * 1024); // 100 MB
    ...
    // Verify data is not all zeros (actual image content)
...
fn test_set_color_transform_then_max_buffer_combined() {
    ...
    decoder.set_max_decoding_buffer_size(50 * 1024 * 1024); // 50 MB
    let data = decoder.decode().unwrap();
    let info = decoder.info().unwrap();
    ...
    // Now try with a tiny buffer - should fail even with color transform set
...
fn test_set_max_decoding_buffer_size_zero() {
    ...
    // Setting buffer size to 0 should cause decode to fail
    let mut decoder = Decoder::new(File::open(&path).unwrap());
    ...
    // Verify a normal decode still works with a fresh decoder
...
fn test_pixel_format_pixel_bytes_values() {
    ...
    // For a standard JPEG, pixel format should be RGB (3 bytes) or L8 (1 byte)
    let pb = info.pixel_format.pixel_bytes();
    assert!(pb == 1 || pb == 3 || pb == 4);

    // For this particular progressive JPEG, it should be RGB
    assert_eq!(pb, 3);
    assert_eq!(info.pixel_format, PixelFormat::RGB24);

    // Verify all PixelFormat variants have sensible pixel_bytes
...
fn test_set_color_transform_multiple_images() {
    ...
    // Decode progressive with RGB transform
    ...
    // Decode ycck image with default
...
fn test_set_max_decoding_buffer_read_info_then_decode() {
    ...
    // Use read_info first, then check if buffer limit applies to decode
    let mut decoder = Decoder::new(File::open(&path).unwrap());
    ...
    // Now try with buffer too small after read_info
    let mut decoder2 = Decoder::new(File::open(&path).unwrap());
    decoder2.set_max_decoding_buffer_size(10);
    // read_info should still work since it doesn't allocate the full buffer
    let read_info_result = decoder2.read_info();
    // Whether read_info respects the limit or not, decode should fail
