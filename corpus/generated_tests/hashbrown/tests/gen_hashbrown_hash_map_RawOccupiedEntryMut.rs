...
fn test_raw_occupied_entry_mut_key_mut() {
    ...
    match map.raw_entry_mut().from_hash(hash, |q| q == "alpha") {
        RawEntryMut::Occupied(mut o) => {
            let key = o.key_mut();
            assert_eq!(key, "alpha");
            // Mutate the key in place (same logical value to keep map consistent for this test)
    ...
    // The key was mutated in place
    assert_eq!(map.len(), 3);
    // We can't look up by "alpha" anymore since the key was mutated
    // but we can iterate and find the modified key
...
fn test_raw_occupied_entry_mut_into_key() {
    ...
    // Mutate the key through the returned mutable reference
    *key_ref = 25;

    // The map still has 3 entries
    assert_eq!(map.len(), 3);
    // The key 20 no longer exists via normal lookup, but 25 might be found via iteration
...
fn test_raw_occupied_entry_mut_insert_key() {
    ...
    assert_eq!(map.len(), 3);
    // The old key "hello" is gone from the entry, replaced by "hello_replaced"
...
fn test_raw_occupied_entry_mut_replace_entry_with_some() {
    ...
    // The closure returned Some, so the entry should still be occupied
    match result {
...
fn test_raw_occupied_entry_mut_replace_entry_with_none() {
    ...
    // The closure returned None, so the entry should be vacant now
    match result {
...
fn test_raw_occupied_entry_mut_combined_workflow() {
    // A comprehensive test that exercises multiple methods in sequence
    ...
    // Step 1: Use get_key_value_mut to modify both key and value
    ...
    // Step 2: Use insert_key to replace a key
    ...
    // Step 3: Use replace_entry_with to conditionally remove
    let hash = compute_hash(map.hasher(), "meta");
    match map.raw_entry_mut().from_hash(hash, |q| q == "meta") {
        RawEntryMut::Occupied(o) => {
            let result = o.replace_entry_with(|_k, v| {
                if v.len() < 2 {
                    None // remove entries with fewer than 2 elements
                } else {
                    Some(v)
                }
            });
            match result {
                RawEntryMut::Vacant(_) => {} // meta had only 1 element, so removed
...
fn test_raw_occupied_entry_mut_key_mut_with_numeric_keys() {
    ...
    assert_eq!(map.len(), 50);
    // The entry with key 25 was mutated to 2500
