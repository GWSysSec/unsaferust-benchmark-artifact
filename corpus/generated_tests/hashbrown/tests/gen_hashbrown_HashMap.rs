...
fn test_hashmap_allocator() {
    ...
    // Verify allocator is accessible after mutations
...
fn test_hashmap_retain() {
    ...
    // Verify all remaining keys are even
    for (&k, &v) in map.iter() {
        assert_eq!(k % 2, 0);
        assert_eq!(v, k * 10);
    }
...
fn test_hashmap_clear() {
    ...
    assert_eq!(map.len(), 0);
    assert!(map.is_empty());
    assert!(map.capacity() >= cap_before); // capacity is not reduced by clear
...
fn test_hashmap_shrink_to() {
    ...
    // Shrink to a capacity that's still above len
    map.shrink_to(20);
    let cap_after = map.capacity();
    assert!(cap_after >= 10); // must hold all elements
...
fn test_hashmap_get_key_value_mut() {
    ...
    // Get existing key-value pair mutably
...
fn test_hashmap_get_many_unchecked_mut() {
    ...
    // Get multiple distinct keys (safe because keys are distinct)
    // SAFETY: all keys are distinct
...
fn test_hashmap_try_insert() {
    ...
    // Insert into empty map should succeed
...
fn test_hashmap_combined_workflow() {
    // Complex workflow combining multiple target APIs
    ...
    // extract_if: remove keys > 15
    let extracted: Vec<_> = map.extract_if(|&k, _| k > 15).collect();
    assert_eq!(extracted.len(), 4); // 18, 21, 24, 27
    assert_eq!(map.len(), 6); // 0, 3, 6, 9, 12, 15
...
fn test_extract_if_partial_consumption() {
    ...
    // Only consume part of the ExtractIf iterator
    ...
    // Map should still have elements (those not yet extracted)
    // At minimum, the two extracted items are gone
    assert!(map.len() <= 18);
    assert!(map.len() >= 8); // at most 10 could have been extracted by the predicate

    // Verify remaining elements are consistent
    for (&k, &v) in map.iter() {
        assert_eq!(v, k * k);
    }
}
...
fn test_retain_with_large_map() {
    ...
    // Retain only prime-ish numbers (simplified: not divisible by 2, 3, or 5)
    map.retain(|&k, _| k % 2 != 0 && k % 3 != 0 && k % 5 != 0 && k > 0);

    // Verify all remaining satisfy the predicate
    for (&k, &v) in map.iter() {
        assert!(k % 2 != 0);
        assert!(k % 3 != 0);
        assert!(k % 5 != 0);
    ...
    assert!(map.len() > 0);
    assert!(map.len() < 1000);
    // Euler's totient-like count: 1000 * (1-1/2) * (1-1/3) * (1-1/5) - 1 (for 0) ≈ 266
    assert!(map.len() > 200);
    assert!(map.len() < 300);
}
...
fn test_shrink_to_boundary_conditions() {
    ...
    // Shrink empty map
    map.shrink_to(0);
    // Capacity might be 0 or some minimum
    assert_eq!(map.len(), 0);

    // Add some elements and shrink to exact len
    for i in 0..8 {
        map.insert(i, i);
    }
    assert_eq!(map.len(), 8);
    ...
    // Verify data after shrink
    for i in 0..8 {
        assert_eq!(map.get(&i), Some(&i));
    }

    // Shrink to value larger than current capacity should be no-op
...
fn test_try_insert_error_details() {
    ...
    // Failed insert - check error contains the attempted value
    let r = map.try_insert("key1".to_string(), 999);
    assert!(r.is_err());
    let err = r.unwrap_err();
    assert_eq!(err.value, 999);
    assert_eq!(*err.entry.key(), "key1");
    // The existing value should still be 42
    assert_eq!(map["key1"], 42);
    assert_eq!(map.len(), 3);

    // The OccupiedError's Display/Debug should work
