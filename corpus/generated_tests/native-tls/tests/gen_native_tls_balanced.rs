//! Balanced unsafe-oriented test for `native-tls` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! native-tls's unsafe lives in the OpenSSL/SChannel FFI backend; without_native
//! attributes only own-crate unsafe and the cycles are spent in the native lib.
...
const SEED: u8 = 0x9a;
const PASSES: u64 = 359;         // load/store passes over the unsafe buffers (RQ1/RQ3)
const SAFE_ITERS: u64 = 4721291; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1161; // safe heap allocations: memory dilution (RQ2)
