...
fn test_use_sni_enabled_builds_connector_successfully() {
    ...
    // Verify the connector is usable by checking it was constructed
    let _connector_ref = &connector;
    assert_eq!(std::mem::size_of_val(&connector) > 0, true);

    // Build another connector with sni disabled
    ...
    // Verify both connectors are distinct allocations
    ...
    // Verify size consistency between both connector instances
...
fn test_use_sni_chained_with_other_builder_methods() {
    // Test chaining use_sni with min_protocol_version and danger_accept_invalid_certs
    ...
    // Now chain with sni disabled and different protocol
    ...
    // Test with max_protocol_version set as well
...
fn test_use_sni_toggle_multiple_times_last_wins() {
    // Toggle use_sni multiple times; the last call should win
    let connector_result = TlsConnector::builder()
        .use_sni(true)
        .use_sni(false)
        .use_sni(true)
    ...
    // Verify both built successfully and are valid objects
    let size1 = std::mem::size_of_val(&connector);
...
fn test_use_sni_with_invalid_cert_does_not_panic() {
    // Ensure that use_sni combined with invalid certificate data handles errors gracefully
    ...
    // Even with a failed cert, building a connector with use_sni should work
    let connector_result = TlsConnector::builder()
        .use_sni(true)
        .danger_accept_invalid_certs(true)
        .build();
    ...
    // Also test identity failure path
...
fn test_use_sni_builder_returns_mutable_reference() {
    // Verify that use_sni returns &mut TlsConnectorBuilder for chaining
    let mut builder = TlsConnector::builder();

    let builder_ref = builder.use_sni(true);
    // We can continue chaining on the returned reference
    ...
    // Test the default connector for comparison
