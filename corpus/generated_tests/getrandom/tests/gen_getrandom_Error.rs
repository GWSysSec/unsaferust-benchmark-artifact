...
fn test_new_custom_many_codes() {
    ...
    // All distinct codes -> distinct errors.
    ...
    // Reconstructing yields equal errors.
...
fn test_successful_calls_no_error_path() {
    // Exercise success paths; ensure they don't produce errors and that
    // contrived Error values via new_custom remain distinguishable.
    let mut buf = [0u8; 32];
    let r = fill(&mut buf);
    assert!(r.is_ok());
    ...
    // Build a few custom errors, just to exercise the API in conjunction.
...
fn test_raw_os_error_none_for_customs_only() {
    // We can only construct Error values via new_custom from the public API.
    // All such values must report raw_os_error() == None.
    ...
    // Sanity: ensure equality is consistent with raw_os_error semantics.
