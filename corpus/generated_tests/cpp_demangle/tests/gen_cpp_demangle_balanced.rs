//! Balanced unsafe-oriented test for `cpp_demangle` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! cpp_demangle's own unsafe is zero-cost pointer/enum casts that emit no
//! executable unsafe instructions; its C ABI lives in the separate c_api crate.
...
const SEED: u8 = 0x5d;
const PASSES: u64 = 1979;         // load/store passes over the unsafe buffers (RQ1/RQ3)
const SAFE_ITERS: u64 = 0; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 0; // safe heap allocations: memory dilution (RQ2)
