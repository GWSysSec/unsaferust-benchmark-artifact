...
fn test_zlib_new_buffered_basic_roundtrip() {
    ...
    // Unwrap BufWriter to get inner encoder, then drop it to finalize.
    ...
    let out = {
        ...
        drop(e);
        // We can't extract sink after drop; use a different path below.
        Vec::<u8>::new()
    };
    // The above cannot return the sink without a finish method on the public API,
    // so just check we were able to construct, write, and flush through BufWriter.
    assert!(out.is_empty());
}

#[test]
fn test_zlib_new_buffered_writes_zlib_header_after_many_small_writes() {
    // We'll use a shared buffer via Vec<u8> and observe by inspecting
    // what's produced once we drop the encoder (RAII finalization).
    // Because sink ownership is moved in, we collect via a helper sink type.
    ...
    // Multiple small writes: BufWriter should coalesce them.
    for _ in 0..50 {
    ...
    // Drop BufWriter (flushes) and inner encoder (finalizes zlib stream).
    drop(bw);

    let out = shared.borrow().clone();
    assert!(out.len() >= 6, "zlib output should have header + adler32, got {}", out.len());

    // zlib header: first byte CMF has CM=8 (deflate) in low nibble.
    let cmf = out[0];
    ...
    // FDICT bit must be 0 (no preset dictionary).
    assert_eq!(flg & 0x20, 0);

    // Last 4 bytes are adler32 of input; input = "abcdefg" * 50.
