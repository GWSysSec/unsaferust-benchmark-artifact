...
fn test_gzip_new_buffered_produces_valid_gzip_header_and_trailer() {
...

    {
        ...
        // BufWriter drops here, flushing to inner GzipEncoder, which finalizes on drop.
    }

    // post-state: a gzip stream has at least a 10-byte header and 8-byte trailer.
    ...
    // Verify ISIZE (little-endian u32 at end) equals input.len() mod 2^32.
    let n = sink.len();
    let isize_val =
    ...
    // Repetitive text should compress to smaller than the input.
    assert!(
        sink.len() < input.len(),
...
fn test_gzip_new_buffered_empty_input_still_valid_frame() {
...

    {
        ...
        // Intentionally do not write anything. Just flush.
        enc.flush().expect("flush on empty must succeed");
    }

    // Even empty input produces a full gzip frame (header + empty stream + trailer).
    ...
    // Framing bytes should not explode the size for an empty payload.
...
fn test_gzip_new_buffered_matches_compress_function_on_same_input() {
    ...
    // Path A: buffered encoder using new_buffered
    ...
    // Path B: top-level compress function with Format::Gzip
