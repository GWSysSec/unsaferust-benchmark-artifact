...
fn test_separated_list0_trailing_separator() {
    // Trailing separator should leave it as remaining
    let mut parser = separated_list0(char(','), alpha1);
    ...
    // Input that doesn't match the element parser at all
...
fn test_separated_list1_fails_on_empty() {
    ...
    // Also fails when input doesn't match element parser
    let result2: IResult<&str, Vec<&str>> = separated_list1(char(','), alpha1).parse("123,abc");
    assert!(result2.is_err());

    // Verify the error is indeed an error
    assert!(result.is_err());
    assert!(result2.is_err());

    // Successful case for contrast
...
fn test_many1_count_basic() {
    ...
    // Multiple matches with leftover
...
fn test_many_till_with_remaining() {
    ...
    // Single element before terminator
...
fn test_fill_fails_insufficient_input() {
    ...
    // Verify buffer was partially filled before failure
    // (implementation detail - the first two should be filled)
    assert_eq!(buf[0], "ab");
    assert_eq!(buf[1], "ab");
    assert_eq!(buf[2], "");
...
fn test_fold_many0_sum_digits() {
    ...
    // With initial value not preceded by separator
...
fn test_fold_many_m_n_basic() {
    ...
    // Exactly 3 matches (within range)
    ...
    // Only 1 match but min is 2 -> error
...
fn test_fold_many_m_n_zero_min() {
    ...
    // No matches, min is 0 so it succeeds
...
fn test_length_count_str() {
    ...
    // length_count parses the count, then calls the item parser `count` times
    // For input "3,hello,world,foo": char_u8 parses "3" -> 3, then we need 3 items each starting with ','
    ...
    // With leftover: parse_lowercase_word stops at uppercase chars,
    // so "cd" is parsed and "EXTRA" remains after the length_count finishes.
