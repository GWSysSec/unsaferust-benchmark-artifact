//! Balanced unsafe-oriented test for `log` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! log's reachable unsafe (set_logger_racy on a static mut) executes only a few
//! instructions and consumes no measurable cycles or heap.
...
const SAFE_ITERS: u64 = 11833328; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1314; // safe heap allocations: memory dilution (RQ2)
