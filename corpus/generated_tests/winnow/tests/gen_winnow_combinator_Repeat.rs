...
fn test_endianness_dispatch() {
    // Declare bytes early so it outlives native_parser
    ...
    // Native produces predictable round-trip
...
fn test_length_take_workflow() {
    // length-prefixed payload: 1-byte length, then that many bytes
