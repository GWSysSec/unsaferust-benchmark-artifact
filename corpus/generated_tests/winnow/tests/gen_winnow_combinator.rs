...
fn test_separated_foldr1_subtraction() {
    // Right-associative: 10-3-2 = 10-(3-2) = 9
...
fn test_fill_into_buffer() {
    ...
    // Failure case - not enough elements
    let mut buf4 = [0u32; 5];
    ...
    // Zero-length buffer always succeeds
    let mut buf5: [u32; 0] = [];
    let mut input5 = "abc";
...
fn test_cond_true_and_false() {
    ...
    // cond(true) with failing inner parser propagates error
    let mut input3 = "abc";
    ...
    // cond(false) with failing inner doesn't run it
    let mut input4 = "abc";
...
fn test_backtrack_err_demotes_cut() {
    ...
    // cut_err normally prevents alt from trying the next branch
    let mut p_cut = alt((
        (
    ...
    // Wrap inner part with backtrack_err to allow alt fallback
    let mut p_bt = alt((
        backtrack_err((
            literal::<_, _, ErrMode<InputError<&str>>>("ab"),
            cut_err(literal("XY")),
    ...
    // backtrack on successful parse passes value through
...
fn test_iterator_basic() {
    ...
    // Iterator that immediately fails: zero iterations
    let input2 = "abc";
