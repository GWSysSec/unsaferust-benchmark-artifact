...
fn test_arc_borrow_from_ptr_scalar_roundtrip() {
    ...
    // get() must yield a reference to the same memory address.
    let got: &i32 = borrow.get();
    assert_eq!(*got, 42);
    assert_eq!(got as *const i32, ptr);

    // clone_arc should produce an Arc aliasing the same allocation.
    let cloned: Arc<i32> = borrow.clone_arc();
    assert_eq!(*cloned, 42);
    assert_eq!(&*cloned as *const i32, ptr);
    assert_eq!(*cloned, *arc);

    // with_arc should receive the same underlying Arc.
    ...
    // Original Arc unaffected by borrows.
    assert_eq!(*arc, 42);
    assert_ne!(*arc, 0);
}
...
fn test_arc_borrow_over_vec_workflow() {
    ...
    // get() yields the backing Vec reference.
    ...
    // clone_arc yields aliased Arc with same backing storage address.
...
fn test_arc_borrow_struct_and_reborrow_chain() {
    ...
    // clone_arc produces a real Arc that can itself be re-borrowed via raw ptr.
    ...
    // Re-cloning through borrow2 yields yet another aliased Arc.
    let a3 = borrow2.clone_arc();
    assert_eq!(&*a3 as *const Point, ptr);
    assert_eq!(a3.label, "origin");

    // Original Arc still intact.
    assert_eq!(arc.x, 3.0);
