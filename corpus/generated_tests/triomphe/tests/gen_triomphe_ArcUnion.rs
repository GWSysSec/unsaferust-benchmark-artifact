...
fn test_arc_union_from_first_basic_workflow() {
    ...
    // Tag checks: must be first, not second.
    assert_eq!(u.is_first(), true);
    assert_eq!(u.is_second(), false);

    // as_second should be None.
    assert!(u.as_second().is_none());

    // as_first yields an ArcBorrow pointing at the same allocation.
    ...
    // clone_arc through borrow still aliases same allocation.
    let a2: Arc<i32> = borrowed.clone_arc();
    assert_eq!(*a2, 42);
    assert_eq!(&*a2 as *const i32, ptr_a);

    // Original Arc unchanged.
    assert_eq!(*a, 42);
}
...
fn test_arc_union_from_second_basic_workflow() {
    ...
    // Tag checks: must be second, not first.
    assert_eq!(u.is_second(), true);
    assert_eq!(u.is_first(), false);

    // as_first should be None.
    assert!(u.as_first().is_none());

    // as_second yields ArcBorrow aliasing the same allocation.
    ...
    // clone_arc produces aliased Arc.
...
fn test_arc_union_ptr_eq_identity_and_inequality() {
    ...
    let a2: Arc<i32> = Arc::new(100); // same value, different allocation
...
fn test_arc_union_borrow_strong_count_progression() {
    ...
    // Both unions should be ptr_eq.
    assert_eq!(ArcUnion::ptr_eq(&u1, &u2), true);

    // as_first succeeds for both, as_second fails for both.
    ...
    // Drop u2; the strong count seen via a fresh borrow should decrease.
    drop(u2);
    let borrow3 = u1.borrow();
    let count_after_drop = ArcUnionBorrowCount::count(&borrow3);
    assert_eq!(count_after_drop, 2);

    // Contents via the original Arc unchanged.
    assert_eq!(a.len(), 5);
    assert_eq!(a[0], 10);
    assert_eq!(a[4], 50);
}

// Helper module to access ArcUnionBorrow::strong_count through the
// importable path. Using the associated function form keeps the call
// unambiguous.
...
fn test_arc_union_mixed_variants_and_borrow_queries() {
    ...
    // Borrow should return counts reflecting the 2 live Arcs per allocation.
    let bf = uf.borrow();
    let bs = us.borrow();
    assert_eq!(ArcUnionBorrowCount::count(&bf), 2);
    assert_eq!(ArcUnionBorrowCount::count(&bs), 2);

    // as_first / as_second alignment with tag.
    ...
    // Cross-variant ptr_eq must be false.
    assert_eq!(ArcUnion::ptr_eq(&uf, &us), false);

    // Self ptr_eq must be true.
...
fn test_arc_union_with_strings_and_vec_roundtrip() {
    // Type parameters where A and B are distinct, non-trivial types.
    ...
    // Retrieve borrows and make strict value checks.
    ...
    // Clone-arc roundtrip yields aliased Arcs.
    ...
    // Strong counts at this point via ArcUnionBorrow::strong_count:
    //   u_s + s_arc + s_back => 3
    //   u_v + v_arc + v_back => 3
    let b_s = u_s.borrow();
    let b_v = u_v.borrow();
    ...
    // Drop clones and verify count drops.
    drop(s_back);
    drop(v_back);
    let b_s2 = u_s.borrow();
    let b_v2 = u_v.borrow();
