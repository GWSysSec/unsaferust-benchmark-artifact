...
fn test_arc_into_raw_offset_roundtrip_scalar() {
    ...
    // Convert to OffsetArc
    let offset: OffsetArc<i32> = Arc::into_raw_offset(arc);

    // OffsetArc should alias the same data.
    ...
    // Clone still points at same allocation.
...
fn test_arc_heap_ptr_preserved_across_offset_conversion() {
    ...
    // Convert to OffsetArc; data pointer preserved.
...
fn test_arc_make_unique_on_sole_reference() {
    ...
    // Only one reference: make_unique should not relocate.
    {
    ...
    // Same allocation after the borrow ends.
...
fn test_arc_make_unique_clones_on_shared_reference() {
    ...
    // Two refs: make_unique must clone.
    {
...
fn test_arc_unwrap_or_clone_unique_returns_inner() {
    ...
    // Sole reference: unwrap_or_clone yields the inner value.
...
fn test_arc_unwrap_or_clone_shared_forces_clone() {
    ...
    // Both refs exist: unwrap_or_clone clones.
    ...
    // Cloned value lives outside the Arc allocation.
    assert_ne!(&owned as *const Data, shared_ptr);

    // Remaining Arc intact.
...
fn test_arc_offset_conversion_chain() {
    ...
    // Multiple round-trips preserve identity.
...
fn test_arc_make_unique_then_unwrap_or_clone() {
    ...
    // First, mutate via make_unique on the sole reference.
    {
    ...
    // Same allocation since it was unique.
    assert_eq!(arc.heap_ptr(), heap_before);
    assert_eq!(arc.len(), 16);
    assert_eq!(arc[7], 7);

    // Still unique, so unwrap_or_clone should not clone.
