//! Balanced unsafe-oriented test for `iri-string` (RQ1 cycles + RQ2 heap + RQ3 insts).
//!
//! iri-string executes only a trace of its own unsafe (unchecked-UTF-8 / slice
//! casts) relative to its large safe parsing workload.
...
const SAFE_ITERS: u64 = 1952764; // safe LCG iters: instruction/cycle dilution (no heap)
const SAFE_ALLOCS: u64 = 1265; // safe heap allocations: memory dilution (RQ2)
