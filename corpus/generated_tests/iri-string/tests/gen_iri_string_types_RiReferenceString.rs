...
fn test_into_iri_with_absolute_iri_reference() {
    // An absolute IRI reference should succeed when converted via into_iri
    ...
    // Pre-state: confirm it's a valid reference string
...
fn test_into_iri_with_relative_reference_fails() {
    // A relative reference should fail when converted via into_iri
...
fn test_into_relative_iri_with_relative_reference() {
    // A relative reference should succeed when converted via into_relative_iri
...
fn test_into_relative_iri_with_absolute_iri_fails() {
    // An absolute IRI should fail when converted via into_relative_iri
...
fn test_remove_password_inline_with_password() {
    // Test removing password from an IRI reference that has userinfo with password
    ...
    // Post-state assertions: password should be removed
    assert!(!ref_string.as_str().contains("secret"));
    assert!(!ref_string.as_str().contains(":secret@"));
    ...
    // The user part should still be present
    assert!(ref_string.as_str().contains("user"));
...
fn test_remove_password_inline_without_password() {
    ...
    // Should remain unchanged since there's no password
    assert_eq!(ref_string.as_str(), input);
...
fn test_remove_nonempty_password_inline_with_nonempty_password() {
    // Test removing non-empty password
    ...
    // Post-state: password should be removed
    assert!(!ref_string.as_str().contains("p4ssw0rd"));
    assert!(!ref_string.as_str().contains(":p4ssw0rd@"));
    assert!(ref_string.as_str().contains("admin"));
