...
fn test_ri_reference_str_to_iri_with_absolute_iri() {
    // An absolute IRI (with scheme) should succeed with to_iri()
    ...
    // to_relative_iri should fail for an absolute IRI
...
fn test_ri_reference_str_to_iri_with_relative_reference() {
    // A relative reference should fail with to_iri()
    ...
    // to_relative_iri should succeed for a relative reference
    let rel_result = ri_ref.to_relative_iri();
    ...
    // Test with path-only relative reference
...
fn test_ri_reference_str_to_iri_uri_spec() {
    // Test with UriSpec for absolute URI
    ...
    // Test with UriSpec for relative URI
...
fn test_ri_reference_str_to_relative_iri_various_forms() {
    ...
    // Authority-based relative reference (no scheme)
    ...
    // Confirm to_iri fails for all of these
...
fn test_mask_password_with_userinfo() {
    // IRI with user:password in authority
    ...
    // The password should be masked (replaced with asterisks or similar)
    ...
    // Verify the masked output still starts with the scheme
    assert!(masked_str.starts_with("http://"));

    // The @ sign should still be present since there's userinfo
    assert!(masked_str.contains("@"));

    // The masked output should not equal the original (password was changed)
    assert_ne!(masked_str, input);
}

#[test]
fn test_mask_password_without_password() {
    // IRI without password (just user, no colon)
    ...
    // Without a password, the output should be the same as input
    ...
    // Relative reference with no authority
...
fn test_mask_password_replace_password() {
    ...
    // Use replace_password to substitute with a custom display value
...
fn test_mask_password_uri_spec() {
    ...
    // Also test replace_password with UriSpec
...
fn test_to_iri_and_to_relative_iri_are_complementary() {
    // For any valid IRI reference, exactly one of to_iri/to_relative_iri should succeed
    let test_cases: Vec<(&str, bool)> = vec![
        ("http://example.com", true),           // absolute
        ("ftp://files.example.com/f", true),    // absolute
        ("urn:isbn:0451450523", true),          // absolute
        ("../relative", false),                 // relative
        ("//authority/path", false),            // relative
        ("?query", false),                      // relative
        ("#frag", false),                       // relative
        ("", false),                            // relative (empty)
    ];
