...
fn test_uri_template_str_new_unchecked_basic() {
    // Test new_unchecked with a known-valid template string
    let template_str = "http://example.com/{var}";
    let result = UriTemplateStr::new(template_str);
    assert!(result.is_ok());
    let validated = result.unwrap();

    // Now use new_unchecked with the same string - should produce equivalent result
    // SAFETY: We already validated the string above
...
fn test_uri_template_str_new_unchecked_various_operators() {
    ...
    // Test with path segment expansion
...
fn test_uri_template_str_expand_dynamic_missing_var() {
    ...
    let mut ctx = SimpleContext::new();
    // Don't insert "name" - it should expand to empty
...
fn test_uri_template_str_expand_dynamic_with_unchecked() {
    // Combine new_unchecked with expand_dynamic
    let template_str = "http://{host}/api/{version}/users";
    // SAFETY: This is a valid URI template string
    ...
    // Also verify variables from unchecked template
...
fn test_uri_template_workflow_end_to_end() {
    // Full workflow: create template, inspect variables, expand
    ...
    // Step 3: Expand with full context
    let mut ctx = SimpleContext::new();
    ctx.insert("host", "api.example.com");
